...
!
	...
	REAL(KIND=LDP), ALLOCATABLE :: dEDdT(:,:)
...
!
	END MODULE MOD_LTE_ROSS_TAB_V3
!
	SUBROUTINE GET_LTE_ROSS_V3(KAP_VAL,KAP_ES,LTE_ED,LTE_dEDdT,ATOM_DEN,TVAL)
	USE SET_KIND_MODULE
	USE MOD_LTE_ROSS_TAB_V3
	IMPLICIT NONE
!
...
!
	   DO I_D=1,N_D
	    CALL MON_INT_FUNS_V2(COEF,ED(:,I_D),TEMP,N_T)
	    dEDdT(:,I_D)=COEF(:,3)
	   END DO
	   DEALLOCATE(COEF)
!
...
!
	D1=T1*LOG(dEDdT(I_T-1,I_D-1))+(1.0_LDP-T1)*LOG(dEDdT(I_T,I_D-1))
	D2=T1*LOG(dEDdT(I_T-1,I_D))+(1.0_LDP-T1)*LOG(dEDdT(I_T,I_D))
	LTE_dEDdT=EXP(T2*D1+(1.0_LDP-T2)*D2)
!
	RETURN
	END
