...
!
	...
	REAL(KIND=LDP) WSE(NLEV,DST:DEND,NPHOT)
	REAL(KIND=LDP) WCR(NLEV,DST:DEND,NPHOT)
...
!
	  ...
	  DO DPTH_INDX=DST,DEND
	    ...
	    SUM_VJ_R=0.0_LDP
	    SUM_VJ_P=0.0_LDP
	    B_RAT=EXP(LOG_DIST(1,J)-LOG_DIST(ION_LEV(IP),J))*(DI(ION_LEV(IP),J)/DI(1,J))
	    DO I=1,NLEV
	      IF(WSE(I,J,IP) .NE. 0.0_LDP)THEN
	        REV_HNST=HNST(I,J)*B_RAT
	        NETR=WSE(I,J,IP)*( REV_HNST*JREC(J)-HN(I,J)*JPHOT(J) )
	        SE(ID)%STEQ(I,J)=SE(ID)%STEQ(I,J)+NETR
	        SUM_SE=SUM_SE+NETR
	        SE(ID)%QFV_R(I,J)=SE(ID)%QFV_R(I,J)+WSE(I,J,IP)*REV_HNST
	        SUM_VJ_R=SUM_VJ_R+WSE(I,J,IP)*REV_HNST
	        SE(ID)%QFV_P(I,J)=SE(ID)%QFV_P(I,J)+WSE(I,J,IP)*HN(I,J)
	        SUM_VJ_P=SUM_VJ_P+WSE(I,J,IP)*HN(I,J)
!
! The next 5 lines update the EHB equation, and compute two matrices
! needed for its variation.
!
	        T1=WSE(I,J,IP)*(HN(I,J)*JPHOT_CR(J)-REV_HNST*JREC_CR(J))
	        T2=WCR(I,J,IP)*(HN(I,J)*JPHOT(J)-REV_HNST*JREC(J))
	        SE(ID)%T_EHB(J)=SE(ID)%T_EHB(J)+PC*(T1+T2)
	        SE(ID)%QFV_R_EHB(I,J)=SE(ID)%QFV_R_EHB(I,J)+WCR(I,J,IP)*REV_HNST
	        SE(ID)%QFV_P_EHB(I,J)=SE(ID)%QFV_P_EHB(I,J)+WCR(I,J,IP)*HN(I,J)
	      END IF
	    END DO
!
	    I=SE(ID)%ION_LEV_TO_EQ_PNT(ION_LEV(IP))
