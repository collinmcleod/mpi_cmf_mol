...
!
	SUBROUTINE DO_PENNING_ION_MPI_V1(HDKT,COMPUTE_BA,DIAG_INDX,ND)
	USE SET_KIND_MODULE
	USE MOD_CMFGEN
	USE STEQ_DATA_MOD
	IMPLICIT NONE
...
!
	REAL(KIND=LDP) RATE
	REAL(KIND=LDP) REV_RATE
	REAL(KIND=LDP) dREV_RATEdT
	REAL(KIND=LDP) REV_RATE_COEF
	REAL(KIND=LDP) T1,T2
!
