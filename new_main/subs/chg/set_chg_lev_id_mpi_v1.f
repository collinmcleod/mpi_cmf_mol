...
!     Y(n+) + X([m-1]+)  <--> Y([n-1]+) + X(M+)
!
	SUBROUTINE SET_CHG_LEV_ID_MPI_V1(ND,LUOUT)
	USE SET_KIND_MODULE
	USE MOD_CMFGEN
	USE CHG_EXCH_MOD_V3
	IMPLICIT NONE
!
	INTEGER ND,LUOUT
!
! Created  18-Jan-2025 : Based on SET_CHH_LEV_ID_V3.
...
!
	...
	IF(IOS .EQ. 0)ALLOCATE (Z_CHG(N_CHG,4),STAT=IOS)
        IF(IOS .EQ. 0)ALLOCATE (AI_AR_CHG(N_CHG,DST:DEND),STAT=IOS)
        IF(IOS .EQ. 0)ALLOCATE (dlnAI_AR_CHG_dlnT(N_CHG,DST:DEND),STAT=IOS)
        IF(IOS .EQ. 0)ALLOCATE (COOL_CHG(N_CHG,DST:DEND),STAT=IOS)
!
        IF(IOS .EQ. 0)ALLOCATE (CHG_ID(N_CHG,4),STAT=IOS)
!
	IF(IOS .NE. 0)THEN
	  WRITE(LUER,*)'Error in SET_CHG_LEV_ID_V4'
