...
!       
       ...
       (LEV_IN_POPS_MOL(J,3) .EQ. 0) .OR. (LEV_IN_POPS_MOL(J,4) .EQ. 0)) THEN
          ...
          IF (COUNT .EQ. 0 .AND. MYPE .EQ. 0) WRITE(LUER1,*) 'Warning from VERIFY_MOL_RXN'
          IF (MYPE .EQ. 0) WRITE(LUER1,*) 'Molecular reaction unavailable, Number is ',J
          COUNT = COUNT+1
       ELSE
          IF ((MOL_Z_CHG(J,1) + MOL_Z_CHG(J,2)) .NE. (MOL_Z_CHG(J,3) + MOL_Z_CHG(J,4)) .AND. (MOL_RXN_TYPE(J) .NE. 3) .AND. MYPE .EQ. 0) THEN
