...
!
             ...
             IF ( (MOL_RXN_TYPE(J) .EQ. 1) .OR. (MOL_RXN_TYPE(J) .EQ. 2) ) THEN
                ...
                DLNK_DLNT = MOL_RXN_COEF(J,2)/300 + (MOL_RXN_COEF(J,3)/TVAL4) !dlnk/dlnt = beta + gamma/T (in Kelvin)
...
!
             IF (PRINT_DEBUG_INFO .AND. UPDATE_BA) THEN
                IF (L .EQ. 1 .AND. J .EQ. 1) THEN
                   ...
                   WRITE(MOL_FILE_NUM,*)'Fourth line: Rate constant K, Scaled K, Forward rate, DRATEDT'
...
MOL_RXN_COEF(J,3)
                WRITE(MOL_FILE_NUM,'(ES15.5,A1,1X,ES15.5,A1,1X,ES15.5,A1,1X,ES15.5)')RATE_K,',',SCALED_K,',',FRD_R,',',DRATEDT
