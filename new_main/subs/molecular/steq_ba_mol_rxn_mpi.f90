...
!
  SUBROUTINE STEQ_BA_MOL_RXN_MPI(POPS,NT,ND,DIAG_INDX,UPDATE_BA)
    USE SET_KIND_MODULE
    USE MOL_RXN_MOD
    USE STEQ_DATA_MOD
    USE MOD_NON_THERM
...
!
    ...
    CHARACTER(30) :: MOL_DEBUG_FILE
    CHARACTER(30) :: MOL_OUT_FILE_SORT='MOLECULAR_REACTIONS_SORTED'
    INTEGER :: IOS,LDIFF,CI1,CI2
    LOGICAL :: IS_OPEN
    REAL(KIND=LDP), ALLOCATABLE :: MOL_RXN_RATES_LOCAL(:,:), MOL_RXN_RATES_ALL(:,:)
    REAL(KIND=LDP), ALLOCATABLE :: MOL_RXN_RATES_SORTED(:,:,:), MOL_RXN_RATES_SORTED_ALL(:,:,:)
    REAL(KIND=LDP), ALLOCATABLE :: T_ALL(:)
!
...
!
    ...
    REAL(KIND=LDP) :: HDKT,DQ_DT1,DQ_DT2,DLNQ_DLNT1,DLNQ_DLNT2,DLN_ETERM1,DLN_ETERM2
!
...
!
    LU_ER=ERROR_LU()
    WRITE(MOL_DEBUG_FILE,'(A,I2.2)') 'MOLECULAR_REACTION_OUTPUT',MYPE
!
    ALLOCATE(MOL_RXN_RATES_ALL(ND,N_MOL_RXN+1),STAT=IOS)
    IF(IOS .EQ. 0) ALLOCATE(MOL_RXN_RATES_LOCAL(DST:DEND,N_MOL_RXN+1),STAT=IOS)
    IF(IOS .EQ. 0) ALLOCATE(MOL_RXN_RATES_SORTED(DST:DEND,N_SPECIES,2),STAT=IOS)
    IF(IOS .EQ. 0) ALLOCATE(MOL_RXN_RATES_SORTED_ALL(ND,N_SPECIES,2),STAT=IOS)
    IF(IOS .EQ. 0) THEN
       MOL_RXN_RATES_ALL(:,:)=0.0_LDP
       MOL_RXN_RATES_SORTED(:,:,:)=0.0_LDP
       MOL_RXN_RATES_LOCAL(:,:)=0.0_LDP
       MOL_RXN_RATES_SORTED_ALL(:,:,:)=0.0_LDP
    ELSE IF (MYPE .EQ. 0) THEN
       WRITE(LU_ER,*) 'Error in STEQ_BA_MOL_RXN: '
       WRITE(LU_ER,*) 'Unable to allocate reaction rate arrays'
    END IF
!
    ALLOCATE(RATE_KS(DST:DEND,N_MOL_RXN),STAT=IOS)
    IF(IOS .EQ. 0) THEN
       RATE_KS(:,:)=0.0_LDP
    ELSE IF (MYPE .EQ. 0) THEN
       WRITE(LU_ER,*) 'Error in STEQ_BA_MOL_RXN: '
...
!
    ...
    END IF
!
    IF (.NOT. ALLOCATED(MTOT_BA)) ALLOCATE(MTOT_BA(N_MOL_LEVS,N_MOL_LEVS+2,DST:DEND),STAT=IOS)
    IF (.NOT. ALLOCATED(MTOT_STEQ)) ALLOCATE(MTOT_STEQ(N_MOL_LEVS,DST:DEND),STAT=IOS)
    IF(IOS .EQ. 0) THEN
       MTOT_BA(:,:,:)=0.0_LDP
       MTOT_STEQ(:,:)=0.0_LDP
    ELSE IF (MYPE .EQ. 0) THEN
...
!
!    WRITE(*,*) 'MYPE: ',MYPE,' N_MOL_RXN: ',N_MOL_RXN,' N_SPECIES: ',N_SPECIES,' DST:DEND: ',DST,DEND
!    WRITE(*,*) 'MYPE: ',MYPE,'Number of available rxns: ',COUNT(MOL_REACTION_AVAILABLE)
    CALL FLUSH(6)
    DO J=1,N_MOL_RXN
...
!
          ...
          IF (E1*E2*E3*E4 .EQ. 0) THEN
             WRITE(LU_ER,*) 'Error in STEQ_BA_MOL_RXN, MYPE IS: ',MYPE
...
!
             ...
             IF ( (MOL_RXN_TYPE(J) .EQ. 1) .OR. (MOL_RXN_TYPE(J) .EQ. 2) ) THEN
                ...
                TR2 = (TVAL4/300.0_LDP)**MOL_RXN_COEF(J,2) !(T/300 K)^beta, T in Kelvin
                RATE_K = MOL_RXN_COEF(J,1)*TR1*TR2 ! the rate constant K
                DLNK_DLNT = MOL_RXN_COEF(J,2)/300.0_LDP + (MOL_RXN_COEF(J,3)/TVAL4) !dlnk/dlnt = beta + gamma/T (in Kelvin)
                ! If T is outside the bounds, the derivative is set to 0
                IF( (T(L) .GE. MOL_RXN_THI(J)) .OR. (T(L) .LE. MOL_RXN_TLO(J)) ) DLNK_DLNT=0.0_LDP
             ELSE IF (MOL_RXN_TYPE(J) .EQ. 3) THEN
                IF (.NOT. ALLOCATED(XKT) .AND. MYPE .EQ. 0) THEN
...
!
!
!DQ_DT,DLNQ_DLNT,DLN_ETERM 1 and 2
  ...
  !                    SET_PART_FUNC2 = .TRUE.
   !                END IF
    !            END IF
                ...
                SCALED_K = SCALED_K*TR3*ATM(ID3)%NXzV
                DLN_ETERM2 = -TR2
                DLNQ_DLNT2 = DQ_DT2/PART_FUNC2
!
!                IF( (T(L) .GE. MOL_RXN_THI(J)) .OR. (T(L) .LE. MOL_RXN_TLO(J)) ) DLNK_DLNT=0.0D0
!
             END IF

             DLNK_DLNT = DLNK_DLNT + DLN_ETERM1 + DLN_ETERM2 - DLNQ_DLNT1 - DLNQ_DLNT2
!
...
!             IF (MYPE .NE. 0 .AND. J .EQ. 1) THEN
!                WRITE(*,*) 'MYPE not 0, first reaction rate at depth ',L
!                WRITE(*,*) 'RATE: ',FRD_R
!                CALL FLUSH(6)
!             END IF
             !
             ...
             !
!             IF (L .EQ. 1) THEN
!                WRITE(LU_ER,*) 'Updating STEQ in STEQ_BA_MOL_RXN_V3'
!                WRITE(LU_ER,*) 'ID1, E1: ',ID1,E1
!                WRITE(LU_ER,*) 'ID2, E2: ',ID2,E2
!                WRITE(LU_ER,*) 'ID3, E3: ',ID3,E3
!                WRITE(LU_ER,*) 'ID4, E4: ',ID4,E4
!                WRITE(LU_ER,*) '----------------------------------'
!             END IF
...
!
             END IF ! UPDATE_BA
!
             MOL_RXN_RATES_LOCAL(L,J)=FRD_R
!
             IF (PRINT_DEBUG_INFO .AND. UPDATE_BA) THEN
                IF (L .EQ. DST .AND. J .EQ. 1) THEN
                   ...
                   WRITE(MOL_FILE_NUM,*)'Fourth line: Rate constant K, Scaled K, Forward rate, DRATEDT, Recorded forward rate'
                   ...
                   WRITE(MOL_FILE_NUM,*)'Fourteenth line: state-dependent temperature terms'
                   WRITE(MOL_FILE_NUM,*) 'DLNK_DLNT'
                   WRITE(MOL_FILE_NUM,*) 'MOL_RXN_RATE_SCALE(J)'
                   WRITE(MOL_FILE_NUM,*)'**************************************************************************'
                   IF (IOS .NE. 0) WRITE(LU_ER,*) 'Error opening molecular debug info file'
                END IF
                IF (L .EQ. DST) WRITE(MOL_FILE_NUM,*) '*********************************************************'
...
MOL_RXN_COEF(J,3)
                WRITE(MOL_FILE_NUM,'(ES15.5,A1,1X,ES15.5,A1,1X,ES15.5,A1,1X,ES15.5,1X,ES15.5)')RATE_K,',',SCALED_K,',',FRD_R,',',DRATEDT,MOL_RXN_RATES_LOCAL(L,J)
                ...
                WRITE(MOL_FILE_NUM,'(A,5X,2ES15.5)'),'DLN_ETERM1/2,',DLN_ETERM1,DLN_ETERM2
                WRITE(MOL_FILE_NUM,'(A,5X,2ES15.5)') 'DLNQ_DLNT1/2:', DLNQ_DLNT1,DLNQ_DLNT2
                WRITE(MOL_FILE_NUM,'(A,5X,ES15.5)') 'DLNK_DLNT: ',DLNK_DLNT
                WRITE(MOL_FILE_NUM,'(A,5X,ES15.5)') 'MOL_RXN_RATE_SCALE: ',MOL_RXN_RATE_SCALE(J)
                WRITE(MOL_FILE_NUM,*)
             END IF
!
             !
...
!
    ...
    END DO
!   Collect all the reaction rates together
!    MOL_RXN_RATES_ALL(:,:) = 0.0
    DO J=1,N_MOL_RXN
       DO I=DST,DEND
          MOL_RXN_RATES_ALL(I,J) = MOL_RXN_RATES_LOCAL(I,J)
 !         IF (MYPE .NE. 0 .AND. MOD(I,5) .EQ. 0 .AND. MOD(J,100) .EQ. 0) THEN
 !            WRITE(*,*) 'MYPE ',MYPE,' MOL RXN RATE:'
 !            WRITE(*,*) 'I,J,RATE: ',I,J,MOL_RXN_RATES_ALL(I,J)
 !         END IF
       END DO
    END DO
    CALL MPI_BARRIER(MPI_COMM_WORLD,IERR)
    CALL MPI_ALLREDUCE(MPI_IN_PLACE,MOL_RXN_RATES_ALL,ND*(N_MOL_RXN+1),MY_MPI_DP,MPI_SUM,MPI_COMM_WORLD,IERR)
...
!    ALLOCATE(T_ALL(ND))
 !   T_ALL = 0.0_LDP
  !  T_ALL(DST:DEND) = T(DST:DEND)
   ! CALL MPI_ALLREDUCE(MPI_IN_PLACE,T_ALL,ND,MY_MPI_DP,MPI_SUM,MPI_COMM_WORLD,IERR)
!
...
!       WRITE(*,*) 'Calling print_mol_info'
    ...
    DO I=1,N_SPECIES
       IF (MOL_ATMID_LIST(I) .NE. 0) THEN
          ...
          ATM(ID1)%MOL_DRXzV(DST:DEND)=0.0_LDP
          ATM(ID1)%MOL_CRXzV(DST:DEND)=0.0_LDP
          DO J=1,N_MOL_RXN
             DO K=1,4
...
!
    RETURN
  END SUBROUTINE STEQ_BA_MOL_RXN_MPI
