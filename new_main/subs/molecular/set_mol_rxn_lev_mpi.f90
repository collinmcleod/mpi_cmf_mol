...
! Edited 22-Sep-2021: Changed treatment of ELECTRON and NOATOM occurrences in reactions
      ...
      SUBROUTINE SET_MOL_RXN_LEV_MPI(ND,LUOUT)
      USE MOD_CMFGEN
      USE MOL_RXN_MOD
      USE SET_KIND_MODULE
...
!      IF (MYPE .EQ. 0) WRITE(LUER1,*) "INCL_MOL_COMP: ",INCL_MOL_COMP
      N_MOL_RXN=0
      N_RXN_OMITTED=0
      DO J=1,N_MOL_RXN_RD
...
!
      ...
      IF(IOS .EQ. 0) ALLOCATE(AI_AR_MOL(N_MOL_RXN,DST:DEND),STAT=IOS)
      IF(IOS .EQ. 0) ALLOCATE(DLNAI_AR_MOL_DLNT(N_MOL_RXN,DST:DEND),STAT=IOS)
      IF(IOS .EQ. 0) ALLOCATE(COOL_MOL(N_MOL_RXN,DST:DEND),STAT=IOS)
      IF(IOS .EQ. 0) ALLOCATE(MOL_ID(N_MOL_RXN,4),STAT=IOS)
...
!
      ...
      END IF
!
      IF (MYPE .EQ. 0) WRITE(LUER1,*)'Allocated main memory for molecular reactions'
!
...
!
      IOS=0
      I=MAXVAL(LEV_CNT)
      IF (MYPE .EQ. 0) WRITE(LUER1,*)'Maximum level count is ',I
...
!*
!                           IF(IPOS .NE. 0)NAME2=NAME2(1:IPOS-1)
                           IF(NAME2 .NE. NAME1 .AND. MYPE .EQ. 0) THEN
                              WRITE(LUER1,*)'Error in SET_MOL_RXN_LEV'
                     ...
                     END IF
                  END DO
                  IF (NAME1 .EQ. ' ' .AND. MYPE .EQ. 0) THEN
            ...
            END IF
         END DO
         IF (MYPE .EQ. 0) WRITE(LUOUT,'(1X,I4,3X,A,3X,2ES12.4)')J,TRIM(STRING1),MOL_RXN_COEF(J,1),MOL_RXN_COEF_RD(MOL_ID(J,1),1)
      END DO
...
!
      ...
      RETURN
      END SUBROUTINE SET_MOL_RXN_LEV_MPI
