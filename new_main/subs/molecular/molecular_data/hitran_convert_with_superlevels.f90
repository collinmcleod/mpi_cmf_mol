...
!     A type which will contain all the data needed for the transition list in the atomic files
      ...
      INTEGER :: V1,J1,N_J_GROUP,I,J,K
      INTEGER, DIMENSION(:), ALLOCATABLE :: SL_ASSIGN
...
!
      ...
      DO COUNTER_I=1, NLEVELS
         ...
         DO COUNTER_K=1,NUM_CHOSEN_ISO
            ...
            IF ((ISO_LOWER_STATE_ENERGIES(COUNTER_K)==TEMP4) .AND. VJMASK(COUNTER_K)) THEN
               ...
               TRANSITION_LIST(COUNTER_L)%EINSTEIN_A=ISO_EINSTEIN_AS(COUNTER_K)
               TRANSITION_LIST(COUNTER_L)%OSCILLATOR_STR=OSC_FROM_EINSTEIN(ISO_EINSTEIN_AS(COUNTER_K),&
   ...
   !Default string for the name of the output file
    WRITE(OUTFILENAME,'(A16,I3.3,A6,I3.3,A4)') "CARB_MONOX_MAXV_", MAXV, "_MAXJ_",MAXJ,"_OSC"
...
https://ui.adsabs.harvard.edu/abs/2015ApJS..216...15L/abstract"
    ...
    WRITE(TEMPSTR1,'(F12.3)') ION_ENERGY_EV/CM1TOEV
    WRITE(12,'(A50,A)') ADJUSTL(TEMPSTR1), " !Ionization energy"

    WRITE(TEMPSTR1,*) " "
    WRITE(12,'(F3.1,A47,A)') 1.0, ADJUSTL(TEMPSTR1), " !Screened nuclear charge"
...
!
    F_TO_S_FILE='COMI_F_TO_S_VIB'
    WRITE(*,'(A,A,A)',ADVANCE="NO") "Choose a name for the f_to_s file [", TRIM(ADJUSTL(F_TO_S_FILE)),"]: "
    READ(*,'(A)') TEMPSTR1
    IF (.NOT. LEN_TRIM(ADJUSTL(TEMPSTR1))==0) THEN
       F_TO_S_FILE=TEMPSTR1
    END IF
!
    DO
       WRITE(*,*) 'How many J states should be grouped together [',MAXJ+1,']?'
       READ(*,'(A)') TEMPSTR1
       IF (INDEX(TEMPSTR1,'LTE') .NE. 0) THEN
          N_J_GROUP = 0
          EXIT
       ELSE IF (.NOT. LEN_TRIM(ADJUSTL(TEMPSTR1))==0) THEN
          READ(TEMPSTR1,*) N_J_GROUP
          IF (MOD(MAXJ+1,N_J_GROUP) .EQ. 0) THEN
             EXIT
          END IF
       END IF
       WRITE(*,*) 'The number of grouped states should divide ',MAXJ+1
    END DO

    IF (N_J_GROUP .NE. 0) THEN
       J = (MAXJ+1)/(N_J_GROUP)
    ELSE
       J=0
    END IF

    ALLOCATE(SL_ASSIGN(NLEVELS))
    DO COUNTER_I=1,NLEVELS
       CALL VJ_FROM_NAME(ENERGY_LEVEL_LIST(COUNTER_I)%LEVEL_NAME,V1,J1)
       IF (N_J_GROUP .NE. 0) THEN
          I = FLOOR(REAL(J1)/REAL(N_J_GROUP))
          SL_ASSIGN(COUNTER_I) = V1*J + I + 1
       ELSE
          SL_ASSIGN(COUNTER_I) = 1
       END IF
    END DO
...
!
    ...
    WRITE(13,'(A50,A)')'6',' !Entry number of link to super level'
    WRITE(13,'(A)') ' '
!
    DO COUNTER_I=1, NLEVELS
       CALL VJ_FROM_NAME(ENERGY_LEVEL_LIST(COUNTER_I)%LEVEL_NAME,V1,J1)
       WRITE(13,'(A33,F5.1,F15.4,F12.5,ES13.3,I8,I8,I8)') ADJUSTL(ENERGY_LEVEL_LIST(COUNTER_I)%LEVEL_NAME), &
            ...
            SL_ASSIGN(COUNTER_I),0,ENERGY_LEVEL_LIST(COUNTER_I)%LEVEL_ID
    END DO
!
    CLOSE(13)
    CALL FLUSH
