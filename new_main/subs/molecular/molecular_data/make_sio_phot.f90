...
PROGRAM MK_PHOT_FILE
  ...
  TYPE(ENERGY_LEVEL), DIMENSION(:), ALLOCATABLE :: SIO_LEVEL_LIST
  ...
  WRITE(CMDATAFILE,'(A)') "SIOMOL_OSC"

  OPEN(UNIT=11,FILE=TRIM(ADJUSTL(CMDATAFILE)),ACTION='READ',IOSTAT=FILEIO)
  ...
  ALLOCATE(SIO_LEVEL_LIST(NLVLSB+1))

  GION1 = 0.0D0
  DO COUNTI=1,NLVLSB
    READ(11,'(A33,F5.1,F15.4,F12.5,F9.3,E13.3,I8,E12.3,E12.3,E12.3)') SIO_LEVEL_LIST(COUNTI)%LEVEL_NAME,&
    SIO_LEVEL_LIST(COUNTI)%LEVEL_G, SIO_LEVEL_LIST(COUNTI)%ENERGY_CMI, SIO_LEVEL_LIST(COUNTI)%FREQ_1015HZ,&
    SIO_LEVEL_LIST(COUNTI)%ENERGY_BELOW_ION_EV, SIO_LEVEL_LIST(COUNTI)%LAMBDA_ANG, &
    SIO_LEVEL_LIST(COUNTI)%LEVEL_ID, SIO_LEVEL_LIST(COUNTI)%ARAD, TREAL1, TREAL2
    GION1 = GION1+SIO_LEVEL_LIST(COUNTI)%LEVEL_G
  END DO

  SIO_LEVEL_LIST(:NLVLSA) = SIO_LEVEL_LIST(2:NLVLSA+1)

  CLOSE(11)
  CALL FLUSH(11)

  SIO_LEVEL_LIST(NLVLSB)%LEVEL_NAME="Ionized_SIO"
  SIO_LEVEL_LIST(NLVLSB)%LEVEL_G=100
  SIO_LEVEL_LIST(NLVLSB)%ENERGY_CMI=92675.2377
  SIO_LEVEL_LIST(NLVLSB)%FREQ_1015HZ=0.0
  SIO_LEVEL_LIST(NLVLSB)%LEVEL_ID = NLVLSB
  SIO_LEVEL_LIST(NLVLSB)%ARAD = 0

  OPEN(UNIT=12,FILE='PHOTSIOMI_A',ACTION='READWRITE',IOSTAT=FILEIO)

  IF (FILEIO>0) STOP "Error opening PHOTSIOMI_A"
  ...
  WRITE(12,*)
  WRITE(12,*) 'Crude photoionization cross-sections for Silicon Monoxide'
  ...
  DO COUNTI=1,NLVLSA !Write out the photoionization information -- this is the simplest possible data, basically
    WRITE(12,*)
    WRITE(12,'(A33,t40,A)') SIO_LEVEL_LIST(COUNTI)%LEVEL_NAME, '!Configuration name'
