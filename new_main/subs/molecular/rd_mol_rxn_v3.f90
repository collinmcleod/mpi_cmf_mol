...
!  Altered 25-Aug-2021: added final steps to read in data according to molecular reaction rules
      ...
      SUBROUTINE RD_MOL_RXN_V3(LUIN,INCL_MOL_RXN,SCALE_FAC,CO_SCALE,DO_NON_THERM,SIL_SCL)
        USE MOL_RXN_MOD
        USE SET_KIND_MODULE
      IMPLICIT NONE
      ...
      REAL(KIND=LDP) CO_SCALE
      LOGICAL DO_NON_THERM
      REAL(KIND=LDP) SIL_SCL
...
! Check whether we are including molecular reactions
      ...
      END IF
!
! Set the scale factors for reaction rates
!
      RXN_SCALE_FAC=SCALE_FAC
      CO_SCALE_FAC = CO_SCALE
      DO_NT = DO_NON_THERM
      SIL_COMP_SCL = SIL_SCL
!
