	...
	INTEGER ID
! 
! Altered 25-Oct-2025 - Modified to work on LOG(DC) space -- still some work needed.
...
!
	...
	REAL(KIND=LDP), ALLOCATABLE :: LOG_ION_POP(:)
...
!
	...
	DO I=1,ND
	 DO J=1,NLEV
	   DC(J,I)=LOG(((T(I)/T_EXC(I))**1.5_LDP))+(HDKT*FEDGE(J)*(1.0_LDP/T_EXC(I)-1.0_LDP/T(I)))
	 END DO
	END DO
!
...
!
	IF(SPECIES .NE. REF_SPECIES)THEN
	  DO I=1,ND
	    ...
	    LOG_ION_POP(I)=LOG(GS_ION_POP(I))-T1-T2
	  END DO
!
! Limit ionization ratio to a factor of 10^10
!
	  DO I=1,ND
	    IF(LOG_ION_POP(I) .LT. LOG(1.0E-10_LDP*GS_ION_POP(I)))THEN
	      T1=LOG(1.0E-10_LDP*GS_ION_POP(I))-LOG_ION_POP(I)
	      DO J=1,NLEV
	        DC(J,I)=DC(J,I)-T1
	      END DO	
	      ION_POP(I)=1.0D-10*GS_ION_POP(I)
	    ELSE
	      ION_POP(I)=EXP(ION_POP(I))
	   END IF
	  END DO
	END IF
!
...
!
	I=ND*NLEV
	CALL CHECK_VEC_NAN(DC,I,REF_SPECIES,NAN_PRES)
	CALL CHECK_VEC_NAN(ION_POP,ND,'ION_POP',NAN_PRES)
...
!
	...
	DEALLOCATE (SIGMA)
	DEALLOCATE (T)
	DEALLOCATE (ION_POP,LOG_ION_POP)
	DEALLOCATE (ED)
