...
!
	    DO J=1,N
	      STEQ(J,K)=STEQ(J,K)*COL_SF(J,K)
	      ERR_EST(K)=MAX(ERR_EST(K),ABS(STEQ(J,K)-OLD_EST(J,K))/(ABS(STEQ(J,K))+ABS(OLD_EST(J,K))+1.0E-16_LDP))
	    END DO
	  END DO
	  CALL TUNE(2,'TRI_DGETRS')
!
	  ...
	  IF(MAXVAL(ERR_EST) .LT. 1.0D-04)EXIT
	  IF(MYPE .EQ. 0)THEN
	    WRITE(6,*)'Maximum error on CM_TRI_BAND iteration',IT_COUNTER,' is (in %): ',200.0_LDP*MAXVAL(ERR_EST)
	  END IF
!
	  OLD_EST=0.0_LDP; OLD_EST(:,DST:DEND)=STEQ(:,DST:DEND); I=N*ND
...
!
	  DO K=DST,DEND
	    IF(K .EQ. 1)THEN
	      DO J=1,N
	        DO I=1,N
	          STEQ(I,K)=STEQ_STORE(I,K)-D_MAT(I,J,K)*OLD_EST(J,K+1)
	        END DO
	      END DO  
	    ELSE IF(K .EQ. ND)THEN
	      DO J=1,N
	        DO I=1,N
	          STEQ(I,K)=STEQ_STORE(I,K)-B_MAT(I,J,K)*OLD_EST(J,K-1)
	        END DO
	      END DO  
	    ELSE
	      DO J=1,N
