...
C
	REAL(KIND=LDP), TARGET :: VCHI(NT,ND)		!VCHI(I,K)=dCHI(K)/dN(I,K)
	REAL(KIND=LDP), TARGET :: VETA(NT,ND)		!VETA(I,K)=dETA(K)/dN(I,K)
	REAL(KIND=LDP), TARGET :: VCHI_NOT_IMP(NT,ND)
	REAL(KIND=LDP), TARGET :: VETA_NOT_IMP(NT,ND)
C
...
C
	...
	REAL(KIND=LDP) LOG_DIST_S(N_DI,ND)
...
!$OMP1 I, L, GENLEV, ALPHA, T1, T2, TCHI1,TCHI2) IF(K_ST .EQ. 1)
	  DO K=K_ST,ND
