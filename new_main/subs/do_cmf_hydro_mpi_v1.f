...
!
	SUBROUTINE DO_CMF_HYDRO_MPI_V1(POPS,MOD_LUM,MOD_TEFF,MOD_LOGG,MOD_MASS,MOD_RSTAR,
...
!
	ELSE IF(HYDRO_OPT .EQ. 'DEFAULT')THEN
	  IF( ABS(TAU_REF-2.0_LDP/3.0_LDP) .GT. 0.001_LDP)THEN
	    ...
	    CALL MPI_ERR_CHECK(1,'Shiutting down code')
	  END IF
	  IF(MYPE .EQ. 0)WRITE(6,'(A)')' Reference radius: based on effective temperature and luminosity of star'
...
!
	ELSE
	  ...
	  CALL MPI_ERR_CHECK(1,'Shiutting down code')
	END IF
	IF(MYPE .EQ. 0)WRITE(6,*)'Reference radius is',REFERENCE_RADIUS
!
!
!
...
!
	...
	IF(GAM_EDD*MAX_ED_ON_NA .GT. 1.0_LDP)THEN
	  ...
	  CALL MPI_ERR_CHECK(1,'Stopping CMFGEN calculation')
	END IF
!
	IF(MYPE .EQ. 0 .AND. VERBOSE_OUTPUT)THEN
...
!
	...
	DO WHILE(ABS(REFERENCE_RADIUS/PREV_REF_RADIUS-1.0_LDP) .GT. 1.0E-05_LDP)
	  ...
	  IF(MYPE .EQ. 0 .AND. VERBOSE_OUTPUT)WRITE(LUV,*)' Beginning new hydro loop'
!
...
!
	  IF(MYPE .EQ. 0)WRITE(6,*)'WIND_PRESENT=',WIND_PRESENT
	  IF(WIND_PRESENT)THEN
!
	        ...
	        IF(T2 .LE. 0.0_LDP)THEN
	          IF(MYPE .EQ. 0)THEN
	            WRITE(LU_ERR,*)'    Connection radius',CONNECTION_RADIUS
                    ...
                    WRITE(LU_ERR,*)'       New GAM_LIM is',GAM_LIM-0.01_LDP
	          END IF
...
!
	  IF(WIND_PRESENT)THEN
	    ...
	    IF(MYPE .EQ. 0)WRITE(6,'(A,3ES14.4)')'SOUND_SPEED',SOUND_SPEED
	  ELSE
...
!	    WRITE(6,*)I,TAU(I)
	    ...
	    IF(I .GT. ND_MAX)THEN
	      ...
	      CALL MPI_ERR_CHECK(1,'MPI run aborting')
	    END IF
!
...
!
	    IF(MYPE .EQ. 0 .AND. VERBOSE_OUTPUT)THEN
	      WRITE(LUV,'(5(5X,A,5X))')'  dPdR','dTAUdR','  T_EST','   dPn',' dTAUn'
	    END IF
100	    P_EST=P(I-1)
	    TAU_EST=TAU(I-1)
	    T_EST=T(I-1)
	    ...
	    dP1=H*dPdR
	    dTAU1=H*dTAUdR
	    IF(MYPE .EQ. 0 .AND. VERBOSE_OUTPUT)WRITE(LUV,'(5ES16.8)')dPdR,dTAUdR,T_EST,dP1,dTAU1
!
	    P_EST=P(I-1)+dP1/2
	    ...
	    dP2=H*dPdR
	    dTAU2=H*dTAUdR
	    IF(MYPE .EQ. 0 .AND. VERBOSE_OUTPUT)WRITE(LUV,'(5ES16.8)')dPdR,dTAUdR,T_EST,dP2,dTAU2
!
	    P_EST=P(I-1)+dP2/2
	    ...
	    dP3=H*dPdR
	    dTAU3=H*dTAUdR
	    IF(MYPE .EQ. 0 .AND. VERBOSE_OUTPUT)WRITE(LUV,'(5ES16.8)')dPdR,dTAUdR,T_EST,dP3,dTAU3
!
	    P_EST=P(I-1)+dP3
	    ...
	    dP4=H*dPdR
	    dTAU4=H*dTAUdR
	    IF(MYPE .EQ. 0 .AND. VERBOSE_OUTPUT)WRITE(LUV,'(5ES16.8)')dPdR,dTAUdR,T_EST,dP4,dTAU4
!
...
!
	    ...
	    IF(.NOT. PLANE_PARALLEL_NO_V)THEN
	      IF(V(I) .GE. V(I-1))THEN
	        GAM_LIM=GAM_LIM-0.01_LDP
	        IF(MYPE .EQ. 0 .AND. VERBOSE_OUTPUT)WRITE(LUV,*)'Resetting GAM_LIM due to -ve velocity gradient'
	        GOTO 100
	      END IF
	    END IF
...
!
	...
	CALL ADJUST_POPS_MPI_V1(POPS,LU_ERR,NEW_ND,NT)
!
...
!
	...
	1                       RM,ES,FM,GAM_FULL
	  ...
	  END IF
	  GAM_FULL=MIN(GAM_LIM,GAM_FULL)
	  IF(MYPE .EQ. 0)WRITE(75,'(ES16.9,8ES12.3)')R_EST,GAM_FULL,ED_ON_NA_EST*GAM_EDD*FM/ES,TAU,FM,RM,KAP_ROSS,KAP_ES,ED_ON_NA_EST
	END IF
!
	RETURN
	END
