...
!
	  ...
	  REAL(KIND=LDP) dED_ON_NA_dTAU_EST
	  REAL(KIND=LDP) dlnT_dTAU_EST
...
!
	...
	END IF
!
	ALLOCATE(COEF(MOD_ND,4),STAT=IOS)
	TB(1:MOD_ND)=OLD_ED/OLD_POP_ATOM
	CALL MON_INT_FUNS_V2(COEF,TB,OLD_TAU,MOD_ND)
	OLD_dED_on_NA_dTAU=COEF(:,3)
	CALL MON_INT_FUNS_V2(COEF,OLD_T,OLD_TAU,MOD_ND)
	OLD_dlnT_dTAU=COEF(:,3)/OLD_T(1:MOD_ND)
	DEALLOCATE(COEF)
!
...
100	    DPTH_INDX=I
	    ...
	    R_EST=R(I-1)-H
	    CALL CMF_HYDRO_DERIVS(TAU_EST,T_EST,ED_ON_NA_EST,ATOM_EST)
	    dV1=H*dVdR
	    dTAU1=H*dTAUdR
!
...
!
	...
	1              dPdR_TERM(I),VdVdR_TERM(I),GRAV_TERM(I),RAD_TERM(I),HYDRO_SUM
	    ...
	    CLOSE(LU)
	  END IF
	  CALL CHECK_HYD(R,V,T,MASS_DENSITY,POP_ATOM,ED,P,GAMMA_FULL,AMU,BC,MU_ATOM,ND,LU)
...
!
	SUBROUTINE CMF_HYDRO_DERIVS(TAU,TEMP,ED_ON_NA,POP_ATOM)
	USE CMF_SUB_SONIC_HYDRO
	USE SET_KIND_MODULE
	IMPLICIT NONE
!
...
! 
	T1=1.0E-10_LDP*BC/MU_ATOM/AMU
	SOUND_SPEED_SQ=T1*(1.0_LDP+ED_ON_NA_EST)*TEMP
	MOD_SOUND_SPEED_SQ=SOUND_SPEED_SQ+0.5_LDP*VTURB*VTURB
	dGTAU=T1*TEMP*dED_ON_NA_dTAU_EST
...
!
	...
	dlnTdR=dlnT_dTAU_EST*dTAUdR
	T1=V_EST/(V_EST*V_EST-MOD_SOUND_SPEED_SQ)
	dVdR=T1*(2*MOD_SOUND_SPEED_SQ/R_EST - SOUND_SPEED_SQ*dlnTdR - dGTAU*dTAUdR - SURF_GRAV*(1.0_LDP-GAM_FULL))
!
	CALL OUT_ESTIMATES('H',STEP_SIZE)
	CALL OUT_ESTIMATES('R_EST',R_EST)
	CALL OUT_ESTIMATES('V_EST',V_EST)
	CALL OUT_ESTIMATES('T_EST',TEMP)
	CALL OUT_ESTIMATES('TAU_EST',TAU_EST)
	T1=MU_ATOM*AMU*ATOM_EST;                           CALL OUT_ESTIMATES('DENS_EST',POP_ATOM)
	CALL OUT_ESTIMATES('ATOM_EST',POP_ATOM)
	CALL OUT_ESTIMATES('ED_ON_NA',ED_ON_NA)
	IF(VTURB .NE. 0)THEN
	  T1=0.5E+10_LDP*MU_ATOM*AMU*POP_ATOM*VTURB*VTURB; CALL OUT_ESTIMATES('    P(TURB)=',T1)
	END IF
	 T1=BC*POP_ATOM*TEMP*(1+ED_ON_NA);                 CALL OUT_ESTIMATES('     P(GAS)=',T1)
	CALL OUT_ESTIMATES('dVdR',dVdR)
	CALL OUT_ESTIMATES('dTAUdR',dTAUdR)
	CALL OUT_ESTIMATES('ROSS_ON_ES',ROSS_ON_ES)
	CALL OUT_ESTIMATES('dlnTdR',dlnTdR)
	...
	T1=-SOUND_SPEED_SQ*dlnTdR; CALL OUT_ESTIMATES('-dC2(T)',T1)
	T1=-dGTAU*dTAUdR         ; CALL OUT_ESTIMATES('-dC2(g)',T1)
...
!
          ...
          dED_ON_NA_dTAU_EST= (1.0_LDP-T1)*OLD_dED_ON_NA_dTAU(INDX-1)+T1*OLD_dED_ON_NA_dTAU(INDX)
          dlnT_dTAU_EST= (1.0_LDP-T1)*OLD_dlnT_dTAU(INDX-1)+T1*OLD_dlnT_dTAU(INDX)
...
!
	SUBROUTINE CHECK_HYD(R,V,T,MASS_DENSITY,POP_ATOM,ED,P,GAMMA_FULL,AMU,BC,MU_ATOM,ND,LU)
	USE SET_KIND_MODULE
	IMPLICIT NONE
	INTEGER ND
!
...
!
	...
	REAL(KIND=LDP) dASQ_dg(ND)
	REAL(KIND=LDP) dASQ_dT(ND)
...
!
	...
	dASQ_dg=ASQ*COEF(:,3)/(1.0_LDP+TA)
!
	CALL MON_INT_FUNS_V2(COEF,T,R,ND)
	dASQ_dT=ASQ*COEF(:,3)/T
!
	CALL MON_INT_FUNS_V2(COEF,V,R,ND)
        dVdR=COEF(:,3)
!
...
!
	WRITE(LU,'(A5,A18,9A14)')'I','R','V','T','P','ASQ','dASQ_dT','dASQ_dg','dPdR_EST','dPdR','Gam'
	DO I=1,ND
	  WRITE(LU,'(I5,ES18.8,9ES14.4)')I,R(I),V(I),T(I),P(I),ASQ(I),dASQ_dT(I),dASQ_dg(I),dPdR_EST(I),dPdR(I),GAMMA_FULL(I)
	END DO
	CLOSE(UNIT=LU)
!
	RETURN
