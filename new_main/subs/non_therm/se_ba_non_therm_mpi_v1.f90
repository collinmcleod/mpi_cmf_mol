...
!
	...
	END DO
	WRITE(6,*)'MYPE=0 -- before MPI_REDUCE',MAXVAL(T1_SUM),MAXVAL(T2_SUM),MPI_COMM_WORLD,IERR
	CALL MPI_BARRIER(MPI_COMM_WORLD,IERR)
	CALL MPI_ALLREDUCE(MPI_IN_PLACE,T1_SUM,ND,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,IERR)
	WRITE(6,*)'After T1 MPI_REDUCE'
	CALL MPI_ALLREDUCE(MPI_IN_PLACE,T2_SUM,ND,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,IERR)
	WRITE(6,*)'After T2 MPI_REDUCE'
!
	IF(MYPE .EQ. 0)THEN
