...
!
	...
	REAL(KIND=LDP) RADIOACTIVE_DECAY_ENERGY_eV(DST:DEND)
...
!
	...
	T2_SUM=0.0_LDP
	DO DPTH_INDX=DST,DEND
	  T1_SUM(DPTH_INDX)=0.0_LDP
	  T2_SUM(DPTH_INDX)=0.0_LDP
	  DO IKT=1,NKT
	    T1_SUM(DPTH_INDX)=T1+YE(IKT,DPTH_INDX)*dXKT(IKT)/SQRT(XKT(IKT))
	    T2_SUM(DPTH_INDX)=T2+XKT(IKT)*YE(IKT,DPTH_INDX)*dXKT(IKT)/SQRT(XKT(IKT))
	  END DO
