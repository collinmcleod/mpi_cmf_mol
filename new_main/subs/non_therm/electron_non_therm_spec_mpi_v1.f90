...
!
	...
	ELSE
	  ...
	  IF(MOD(NT_ITERATION_COUNTER-1,NON_THERMAL_IT_CNTRL) .NE. 0)THEN
	    CALL TUNE(2,'NT_TOP')
	    RETURN
	  END IF
	END IF
...
!
	...
	I=0; J=100
	IF(MYPE .EQ. NTHREAD-1)CALL MPI_SEND(LELEC,NKT,MY_MPI_DP,I,J,MPI_COMM_WORLD,IERR)
	IF(MYPE .EQ. 0)THEN
	  I=NTHREAD-1
	  CALL MPI_RECV(LELEC,NKT,MY_MPI_DP,I,J,MPI_COMM_WORLD,REC_STATUS,IERR)
	END IF
	IF(MYPE .EQ. 0)THEN
	  FMT='(1x,8ES20.10)'
