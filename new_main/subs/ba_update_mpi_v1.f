...
!
	END MODULE BA_J_DATA_MOD_MPI_V1
!
...
!
	REAL(KIND=LDP) COMP_VEC(NT,NUM_BNDS,DST:DEND)
!
...
!
	IF(.NOT. ALLOCATED(VJ_R))THEN
	  ...
	  IF(IOS .EQ. 0)ALLOCATE (VJ_T(NT,NUM_BNDS,DST:DEND),STAT=IOS)
...
!
	IF(NEW_CONT .AND. .NOT. FINAL_FREQ)THEN
	  ...
	  T2=FQW/NU
	  WRITE(6,*)'X',DST,DEND,SIZE(VJ_R,1),SIZE(VJ_R,2),SIZE(VJ_R,3),
	1                  SIZE(VJ,1),SIZE(VJ,2),SIZE(VJ,3)
	  WRITE(6,*)DST,VJ_R(1,1,DST),VJ_R(NT,1,DST),VJ_R(1,1,DEND),VJ_R(NT,1,DEND)
	  WRITE(6,*)DEND,VJ(1,1,DST),VJ(NT,1,DST),VJ(1,1,DEND),VJ(NT,1,DEND)
	  DO L=DST,DEND
...
!
	  ...
	  END DO
!
! NB: We use VJ_P to compute COMP_VEC as this is defined a Int[ (VJ/v) dv].
!            RJ_SUM is defined in the same way (i..e., Int[ (J/v) dv]
!
	  DO L=DST,DEND
	    BNDST=MAX( 1+DIAG_INDX-L, 1 )
	    BNDEND=MIN( ND+DIAG_INDX-L, NUM_BNDS )
