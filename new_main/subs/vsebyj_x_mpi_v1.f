...
!
	SUBROUTINE VSEBYJ_X_MPI_V1(ID,WSE_X,
...
!
	INTEGER DST,DEND
!
	REAL(KIND=LDP) WSE_X(N_A,DST:DEND)		!Quadrature weights (incl. cross. sec.)
!
! _A refers to quantities associated with the atom WITH super levels.
!
	REAL(KIND=LDP) HN_A(N_A,DST:DEND)		!Pops. of ith ionzation stage
	REAL(KIND=LDP) HNST_A(N_A,DST:DEND)		!LTE   "    "  "      "       "
	REAL(KIND=LDP) dlnHNST_AdlnT(N_A,DST:DEND)	
!
! _B refers to quantities associated with the FULL atom of the next
!  ionization stage.
!
	REAL(KIND=LDP) HN_B(N_B,DST:DEND)		!Pops. of (i+1)th ionization stage
	REAL(KIND=LDP) HNST_B(N_B,DST:DEND)		!LTE   "   "    "       "        "
