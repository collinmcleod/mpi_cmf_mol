...
!
	SUBROUTINE SET_TWO_PHOT_ATM_MPI_V1(SPECIES,ID,
	1            HNST_S,N_S,
	1            HNST_F_ON_S,LEVEL_NAME,EDGE_F,G_F,F_TO_S,N_F,
	1            DST,DEND,ZION,EQSPEC,SPECIES_PRESENT)
	USE SET_KIND_MODULE
	USE TWO_PHOT_MOD
	IMPLICIT NONE
!
! Created 18-Aug-2024: Bases on set_two_phot_v3.f
...
C
	  ALLOCATE (FS_RAT_LOW(DST:DEND,N_TWO))
	  ALLOCATE (FS_RAT_UP(DST:DEND,N_TWO))
	  ALLOCATE (UP_RATE_TWO(DST:DEND,N_TWO))
	  ALLOCATE (DOWN_RATE_TWO(DST:DEND,N_TWO))
	  ALLOCATE (PHOT_OC_TWO(DST:DEND,N_TWO))
C
	  INITIALIZE_TWO=.TRUE.
	END IF
C
...
!
	...
	1              LEVEL_NAME(I) .EQ. A_LOW_NAME_TWO(J) )THEN
	          ...
	          G_LOW_TWO(J)=G_F(I_F)
	          FS_RAT_LOW(DST:DEND,J)=HNST_F_ON_S(I_F,DST:DEND)
	       END IF
	    END DO
!
...
!
	...
	1                 LEVEL_NAME(I) .EQ. A_UP_NAME_TWO(J))THEN
	          ...
	          FS_RAT_UP(DST:DEND,J)=HNST_F_ON_S(I_F,DST:DEND)
!
