...
!
	      DO K=DST,DEND
	        IF(K .EQ. DST .AND. K .NE. 1)THEN
	          ITAG=MYPE
	          CALL MPI_RECV(STEQ_WRK_VEC,N,MPI_DOUBLE_PRECISION,ITHREAD-1,MYPE,ITAG,MPI_COMM_WORLD,REC_STATUS,IERR)
	        ELSE IF(K .NE. 1)THEN
...
!
	        ...
	        IF(K .EQ. DEND .AND. K .LT. ND)THEN
	          ITAG=MYPE+1
	          CALL MPI_SEND(STEQ(1,DEND),N,MPI_DOUBLE_PRECISION,ITHREAD+1,MYPE,ITAG,MPI_COMM_WORLD,IERR)
	        END IF
	...
	DO ITHREAD=NTHREAD-1,0,-1
	  IF(MYPE .EQ. ITHREAD)THEN
	    DO K=MIN(ND-1,DEND),DST,-1
	      IF(K .EQ. DEND)THEN
	        ITAG=MYPE
	        CALL MPI_RECV(STEQ_WRK_VEC,N,MPI_DOUBLE_PRECISION,ITHREAD+1,ITAG,MPI_COMM_WORLD,REC_STATUS,IERR)
	      ELSE
	        STEQ_WRK_VEC=STEQ(:,K+1)
	      END IF
!
...
!
	...
	1                 INT_ONE,DP_ONE,STEQ(1,K),INT_ONE)
	      ...
	      IF(K .EQ. DST .AND. DST .NE. 1)THEN
	        ITAG=MYPE-1
	        CALL MPI_SEND(STEQ(1,DST),N,MPI_DOUBLE_PRECISION,ITHREAD-1,ITAG,MPI_COMM_WORLD,IERR)
	      END IF
	    END DO
	  END IF
	  CALL MPI_BARRIER(MPI_COMM_WORLD,IERR)
...
!
	        IF(K .EQ. DST)THEN
	          I=N*(N+1); ITAG=MYPE
	          CALL MPI_RECV(PREV_D_MAT,I,MPI_DOUBLE_PRECISION,MYPE-1,ITAG,MPI_COMM_WORLD,REC_STATUS,IERR)
...
!
	...
	1                POPS(:,K),REPLACE_EQ,ZERO_STEQ,N,NION,DEPTH_INDX,'D')
	      ...
	      IF(K .EQ. DEND .AND. K .NE. ND)THEN
	        ...
	        CALL MPI_SEND(PREV_D_MAT,I,MPI_DOUBLE_PRECISION,MYPE+1,ITAG,MPI_COMM_WORLD,IERR)
	      END IF
!
	      IF(WR_BA_INV)THEN
