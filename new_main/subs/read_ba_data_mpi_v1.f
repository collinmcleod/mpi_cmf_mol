...
!
	...
	RETURN
!
300	WRITE(LUER,*)'Error opening '//TRIM(FILE_NAME)//' in READBA'
	...
	RETURN
!
400	WRITE(LUER,*)'Error reading from '//TRIM(FILE_NAME)//' in READBA'
        WRITE(LUER,*)'IOSTAT=',IOS
	SUCCESSFUL_READ=.FALSE.
	INQUIRE(UNIT=LU,OPENED=FILE_OPEN)
	IF(FILE_OPEN)CLOSE(UNIT=LU)
	RETURN
!
500	WRITE(LUER,*)'Error opening logical unit to recall :',TRIM(FILE_NAME)
        WRITE(LUER,*)'IOSTAT=',IOS
	SUCCESSFUL_READ=.FALSE.
	INQUIRE(UNIT=LU,OPENED=FILE_OPEN)
	IF(FILE_OPEN)CLOSE(UNIT=LU)
	RETURN
!
600	WRITE(LUER,*)'Error on reading SE(ID)%BA in READ_BA_DATA_V2: ',TRIM(FILE_NAME)
        WRITE(LUER,*)'IOSTAT=',IOS
	SUCCESSFUL_READ=.FALSE.
	INQUIRE(UNIT=LU,OPENED=FILE_OPEN)
	IF(FILE_OPEN)CLOSE(UNIT=LU)
	RETURN
!
605	WRITE(LUER,*)'Error on reading BA_ED in READ_BA_DATA_V2: ',TRIM(FILE_NAME)
        WRITE(LUER,*)'IOSTAT=',IOS
	SUCCESSFUL_READ=.FALSE.
	INQUIRE(UNIT=LU,OPENED=FILE_OPEN)
	IF(FILE_OPEN)CLOSE(UNIT=LU)
	RETURN
!
610	WRITE(LUER,*)'Error on reading BA_T in READ_BA_DATA_V2: ',TRIM(FILE_NAME)
        WRITE(LUER,*)'IOSTAT=',IOS
	SUCCESSFUL_READ=.FALSE.
	INQUIRE(UNIT=LU,OPENED=FILE_OPEN)
	IF(FILE_OPEN)CLOSE(UNIT=LU)
