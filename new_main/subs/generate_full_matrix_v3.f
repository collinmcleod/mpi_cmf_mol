...
!
	LUOUT=487
	IF(K .GE. 48**2)THEN				!**2 prevents writing
	  WRITE(FILENAME,'(I2,A,I1)')K,'_',BAND_INDX
	  FILENAME='BA_ASCI_D_'//TRIM(FILENAME)
	  OPEN(UNIT=LUOUT,FILE=FILENAME,STATUS='UNKNOWN')
	    CALL WR2D_MA(C_MAT,NT,NT,FILENAME,LUOUT)
	  CLOSE(LUOUT)
	END IF
!
...
!
	...
	IF(K .EQ. 58 .AND. DIAG_BAND .AND. K .LE. ND)THEN
	  OPEN(UNIT=LUOUT,FILE='BA_ASCI_N_D58',STATUS='UNKNOWN')
	    CALL WR2D_MA(POPS(1,K),NT,1,'POPS_D1',LUOUT)
	    CALL WR2D_MA(STEQ_VEC,NT,1,'STEQ_VEC_D1',LUOUT)
	    CALL WR2D_MA(C_MAT,NT,NT,'C_MAT_D1',LUOUT)
	  CLOSE(UNIT=LUOUT)
	END IF
	IF(K .EQ. 15 .AND. DIAG_BAND .AND. K .LE. ND)THEN
	  OPEN(UNIT=LUOUT,FILE='BA_ASCI_N_D15',STATUS='UNKNOWN')
	    CALL WR2D_MA(POPS(1,K),NT,1,'POPS_D1',LUOUT)
	    CALL WR2D_MA(STEQ_VEC,NT,1,'STEQ_VEC_D1',LUOUT)
	    CALL WR2D_MA(C_MAT,NT,NT,'C_MAT_D1',LUOUT)
	  CLOSE(UNIT=LUOUT)
	END IF
	IF(K .EQ. 52 .AND. DIAG_BAND .AND. K .LE. ND)THEN
	  OPEN(UNIT=LUOUT,FILE='BA_ASCI_N_D52',STATUS='UNKNOWN')
	    CALL WR2D_MA(POPS(1,K),NT,1,'POPS_D1',LUOUT)
	    CALL WR2D_MA(STEQ_VEC,NT,1,'STEQ_VEC_D1',LUOUT)
	    CALL WR2D_MA(C_MAT,NT,NT,'C_MAT_D1',LUOUT)
	  CLOSE(UNIT=LUOUT)
