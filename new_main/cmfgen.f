...
!
! Local variables helpful for treating molecules:
!
...
!
! Initialize the variables used to identify molecules:
!
	IS_MOLECULE = .FALSE.
	MOL_ATOMS(:,:) = 'NONE'
	MOL_ATM_NUMS(:,:) = 0
...
!
!
!       I have added the following molecules.
!       For most molecules, values for atomic number are arbitrary, starting at 101 and increasing sequentially
!       All molecular species have abbreviations ending in 'M'
!       Listed solar abundances for molecules are arbitrary, chosen to be very small
!       Listed masses for molecules are accurate, derived from NIST (PML atomic weights)
!
	ID=ID+1
	AT_NO(ID)=101.0E+00;	    AT_MASS(ID)=28.01E+00	        !Molecule - Carbon Monoxide
	SPECIES(ID)='COMOL';	    SPECIES_ABR(ID)='COM'
	SOL_ABUND_HSCL(ID)=1.0E+00
	IS_MOLECULE(ID) = .TRUE.
	MOL_ATOMS(ID,1) = 'CARB';   MOL_ATM_NUMS(ID,1)=1
	MOL_ATOMS(ID,2) = 'OXY';    MOL_ATM_NUMS(ID,2)=1
	
!
  	ID=ID+1
	AT_NO(ID)=102.0E+00;	    AT_MASS(ID)=24.02E+00	        !Molecule - Diatomic Carbon
	SPECIES(ID)='C2MOL';	    SPECIES_ABR(ID)='C2M'
	SOL_ABUND_HSCL(ID)=1.0E+00
	IS_MOLECULE(ID) = .TRUE.
	MOL_ATOMS(ID,1) = 'CARB';   MOL_ATM_NUMS(ID,1)=2
	MOL_ATOMS(ID,2) = 'NONE';   MOL_ATM_NUMS(ID,2)=0
!
	ID=ID+1
	AT_NO(ID)=103.0E+00;	    AT_MASS(ID)=31.998E+00        !Molecule - Diatomic Oxygen
	SPECIES(ID)='O2MOL';	    SPECIES_ABR(ID)='O2M'
	SOL_ABUND_HSCL(ID)=1.0E+00
	IS_MOLECULE(ID) = .TRUE.
	MOL_ATOMS(ID,1) = 'OXY';   MOL_ATM_NUMS(ID,1)=2
	MOL_ATOMS(ID,2) = 'NONE';  MOL_ATM_NUMS(ID,2)=0
!
	ID=ID+1
	AT_NO(ID)=104.0E+00;	    AT_MASS(ID)=44.008E+00        !Molecule - Carbon Dioxide
	SPECIES(ID)='CO2MOL';	    SPECIES_ABR(ID)='CO2M'
	SOL_ABUND_HSCL(ID)=1.0E+00
	IS_MOLECULE(ID) = .TRUE.
	MOL_ATOMS(ID,1) = 'CARB';   MOL_ATM_NUMS(ID,1)=1
	MOL_ATOMS(ID,2) = 'OXY';    MOL_ATM_NUMS(ID,2)=2
!
	ID=ID+1
	AT_NO(ID)=105.0E+00;	    AT_MASS(ID)=40.018E+00        !Molecule - Dicarbon Monoxide
	SPECIES(ID)='C2OMOL';	    SPECIES_ABR(ID)='C2OM'
	SOL_ABUND_HSCL(ID)=1.0E+00
	IS_MOLECULE(ID) = .TRUE.
	MOL_ATOMS(ID,1) = 'CARB';   MOL_ATM_NUMS(ID,1)=2
	MOL_ATOMS(ID,2) = 'OXY';    MOL_ATM_NUMS(ID,2)=1
!
	ID=ID+1
	AT_NO(ID)=106.0E+00;	    AT_MASS(ID)=44.08E+00         !Molecule - Silicon Monoxide
	SPECIES(ID)='SIOMOL';	    SPECIES_ABR(ID)='SIOM'
	SOL_ABUND_HSCL(ID)=1.0E+00
	IS_MOLECULE(ID) = .TRUE.
	MOL_ATOMS(ID,1) = 'SIL';    MOL_ATM_NUMS(ID,1)=1
	MOL_ATOMS(ID,2) = 'OXY';    MOL_ATM_NUMS(ID,2)=1
!
	ID=ID+1
	AT_NO(ID)=107.0E+00;	    AT_MASS(ID)=60.15E+00         !Molecule - Silicon Monosulfide
	SPECIES(ID)='SISMOL';	    SPECIES_ABR(ID)='SISM'
	SOL_ABUND_HSCL(ID)=1.0E+00
	IS_MOLECULE(ID) = .TRUE.
	MOL_ATOMS(ID,1) = 'SIL';    MOL_ATM_NUMS(ID,1)=1
	MOL_ATOMS(ID,2) = 'SUL';    MOL_ATM_NUMS(ID,2)=1
!
	ID=ID+1
	AT_NO(ID)=108.0E+00;	    AT_MASS(ID)=30.01E+00         !Molecule - Nitrogen Monoxide
	SPECIES(ID)='NOMOL';	    SPECIES_ABR(ID)='NOM'
	SOL_ABUND_HSCL(ID)=1.0E+00
	IS_MOLECULE(ID) = .TRUE.
	MOL_ATOMS(ID,1) = 'NIT';    MOL_ATM_NUMS(ID,1)=1
	MOL_ATOMS(ID,2) = 'OXY';    MOL_ATM_NUMS(ID,2)=1
!
	ID=ID+1
	AT_NO(ID)=109.0E+00;	    AT_MASS(ID)=48.064E+0         !Molecule - Sulfur Monoxide
	SPECIES(ID)='SOMOL';	    SPECIES_ABR(ID)='SOM'
	SOL_ABUND_HSCL(ID)=1.0E+00
	IS_MOLECULE(ID) = .TRUE.
	MOL_ATOMS(ID,1) = 'SUL';    MOL_ATM_NUMS(ID,1)=1
	MOL_ATOMS(ID,2) = 'OXY';    MOL_ATM_NUMS(ID,2)=1
!
	ID=ID+1
	AT_NO(ID)=110.0E+00;	    AT_MASS(ID)=40.11E+0         !Molecule - Silicon Carbide
	SPECIES(ID)='SICMOL';	    SPECIES_ABR(ID)='SICM'
	SOL_ABUND_HSCL(ID)=1.0E+00
	IS_MOLECULE(ID) = .TRUE.
	MOL_ATOMS(ID,1) = 'SIL';    MOL_ATM_NUMS(ID,1)=1
	MOL_ATOMS(ID,2) = 'CARB';    MOL_ATM_NUMS(ID,2)=1
!
	ID=ID+1
	AT_NO(ID)=111.0E+00;	    AT_MASS(ID)=19.093E+0         !Molecule - Silicon Hydride
	SPECIES(ID)='SIHMOL';	    SPECIES_ABR(ID)='SIHM'
	SOL_ABUND_HSCL(ID)=1.0E+00
	IS_MOLECULE(ID) = .TRUE.
	MOL_ATOMS(ID,1) = 'SIL';    MOL_ATM_NUMS(ID,1)=1
	MOL_ATOMS(ID,2) = 'HYD';    MOL_ATM_NUMS(ID,2)=1
!
	ID=ID+1
	AT_NO(ID)=112.0E+00;	    AT_MASS(ID)=42.092E+0         !Molecule - Silicon Nitride
	SPECIES(ID)='SINMOL';	    SPECIES_ABR(ID)='SINM'
	SOL_ABUND_HSCL(ID)=1.0E+00
	IS_MOLECULE(ID) = .TRUE.
	MOL_ATOMS(ID,1) = 'SIL';    MOL_ATM_NUMS(ID,1)=1
	MOL_ATOMS(ID,2) = 'NIT';    MOL_ATM_NUMS(ID,2)=1
!
	ID=ID+1
	AT_NO(ID)=113.0E+00;	    AT_MASS(ID)=42.092E+0         !Molecule - Hydroxyl
	SPECIES(ID)='OHMOL';	    SPECIES_ABR(ID)='OHM'
	SOL_ABUND_HSCL(ID)=1.0E+00
	IS_MOLECULE(ID) = .TRUE.
	MOL_ATOMS(ID,1) = 'OXY';    MOL_ATM_NUMS(ID,1)=1
	MOL_ATOMS(ID,2) = 'HYD';    MOL_ATM_NUMS(ID,2)=1
!
	ID=ID+1
	AT_NO(ID)=114.0E+00;	    AT_MASS(ID)=64.066E+0         !Molecule - Sulfur Dioxide
	SPECIES(ID)='SO2MOL';	    SPECIES_ABR(ID)='SO2M'
	SOL_ABUND_HSCL(ID)=1.0E+00
	IS_MOLECULE(ID) = .TRUE.
	MOL_ATOMS(ID,1) = 'SUL';    MOL_ATM_NUMS(ID,1)=1
	MOL_ATOMS(ID,2) = 'OXY';    MOL_ATM_NUMS(ID,2)=2
!
	ID=ID+1
	AT_NO(ID)=115.0E+00;	    AT_MASS(ID)=26.018E+0         !Molecule - Cyanide
	SPECIES(ID)='CNMOL';	    SPECIES_ABR(ID)='CNM'        !CN is isoelectronic with CO
	SOL_ABUND_HSCL(ID)=1.0E+00
	IS_MOLECULE(ID) = .TRUE.
	MOL_ATOMS(ID,1) = 'CARB';    MOL_ATM_NUMS(ID,1)=1
	MOL_ATOMS(ID,2) = 'NIT';     MOL_ATM_NUMS(ID,2)=1
!
	ID=ID+1
	AT_NO(ID)=116.0E+00;	    AT_MASS(ID)=13.019E+0         !Molecule - Methyladine
	SPECIES(ID)='CHMOL';	    SPECIES_ABR(ID)='CHM'        
	SOL_ABUND_HSCL(ID)=1.0E+00
	IS_MOLECULE(ID) = .TRUE.
	MOL_ATOMS(ID,1) = 'CARB';    MOL_ATM_NUMS(ID,1)=1
	MOL_ATOMS(ID,2) = 'HYD';     MOL_ATM_NUMS(ID,2)=1
!
	ID=ID+1
	AT_NO(ID)=117.0E+00;	    AT_MASS(ID)=33.073E+0         !Molecule - Hydrogen Monosulfide
	SPECIES(ID)='HSMOL';	    SPECIES_ABR(ID)='HSM'
	SOL_ABUND_HSCL(ID)=1.0E+00
	IS_MOLECULE(ID) = .TRUE.
	MOL_ATOMS(ID,1) = 'HYD';    MOL_ATM_NUMS(ID,1)=1
	MOL_ATOMS(ID,2) = 'SUL';     MOL_ATM_NUMS(ID,2)=1
!
	ID=ID+1
	AT_NO(ID)=118.0E+00;	    AT_MASS(ID)=44.07E+0          !Molecule - Carbon Monosulfide
	SPECIES(ID)='CSMOL';	    SPECIES_ABR(ID)='CSM'
	SOL_ABUND_HSCL(ID)=1.0E+00
	IS_MOLECULE(ID) = .TRUE.
	MOL_ATOMS(ID,1) = 'CARB';    MOL_ATM_NUMS(ID,1)=1
	MOL_ATOMS(ID,2) = 'SUL';     MOL_ATM_NUMS(ID,2)=1
!
	ID=ID+1
	AT_NO(ID)=119.0E+00;	    AT_MASS(ID)=60.084E+0          !Molecule - Silicon Dioxide
	SPECIES(ID)='SIO2MOL';	    SPECIES_ABR(ID)='SIO2M'
	SOL_ABUND_HSCL(ID)=1.0E+00
	IS_MOLECULE(ID) = .TRUE.
	MOL_ATOMS(ID,1) = 'SIL';     MOL_ATM_NUMS(ID,1)=1
	MOL_ATOMS(ID,2) = 'OXY';     MOL_ATM_NUMS(ID,2)=2
!
	ID=ID+1
	AT_NO(ID)=120.0E+00;	    AT_MASS(ID)=28.0134E+0          !Molecule - Silicon Dioxide
	SPECIES(ID)='N2MOL';	    SPECIES_ABR(ID)='N2M'
	SOL_ABUND_HSCL(ID)=1.0E+00
	IS_MOLECULE(ID) = .TRUE.
	MOL_ATOMS(ID,1) = 'NIT';     MOL_ATM_NUMS(ID,1)=2
	MOL_ATOMS(ID,2) = 'NONE';    MOL_ATM_NUMS(ID,2)=0
!
	ID=ID+1
	AT_NO(ID)=121.0E+00;	    AT_MASS(ID)=15.015E+0          !Molecule - Nitrogen Hydride
	SPECIES(ID)='NHMOL';	    SPECIES_ABR(ID)='NHM'
	SOL_ABUND_HSCL(ID)=1.0E+00
	IS_MOLECULE(ID) = .TRUE.
	MOL_ATOMS(ID,1) = 'NIT';     MOL_ATM_NUMS(ID,1)=1
	MOL_ATOMS(ID,2) = 'HYD';     MOL_ATM_NUMS(ID,2)=1
...
!
	    IF(NS .NE. 0)THEN
	      ...
	      CALL FDG_F_TO_S_NS_V2(NF,NS,NV,FL_OPTION,SL_OPTION,dE_OPTION,IL_OPTION,LU_IN,TEMP_KEY,IS_MOLECULE(ISPEC))
	      ID=ID+1
...
!
	  IF(AT_LEAST_ONE_ION_PRES)THEN
	    ...
	    IF (SPECIES(ISPEC) .EQ. 'COMOL') THEN
	       EQ_SPECIES(ISPEC) = EQ_SPECIES(CARBSPEC)
	    ELSE IF (SPECIES(ISPEC) .EQ. 'C2MOL') THEN
	       EQ_SPECIES(ISPEC) = EQ_SPECIES(CARBSPEC)
	    ELSE IF (SPECIES(ISPEC) .EQ. 'O2MOL') THEN
	       EQ_SPECIES(ISPEC) = EQ_SPECIES(OXYSPEC)
...
!
	  ...
	  IF(IOS .EQ. 0)ALLOCATE (ATM(ID)%MOL_CRXzV(DST:DEND),STAT=IOS)
	  IF(IOS .EQ. 0)ALLOCATE (ATM(ID)%MOL_DRXzV(DST:DEND),STAT=IOS)
	  ATM(ID)%MOL_CRXzV(:)=0.0D0
	  ATM(ID)%MOL_DRXzV(:)=0.0D0
...
!
	    ...
	    IF(IOS .EQ. 0)ALLOCATE (ROOT(ID)%MOL_CRXzV(ND),STAT=IOS)
	    IF(IOS .EQ. 0)ALLOCATE (ROOT(ID)%MOL_DRXzV(ND),STAT=IOS)
