...
!
	END IF				!if(grid)
!
	CALL MPI_BCAST(T, ND,MY_MPI_DP,IZERO,MPI_COMM_WORLD,IERR)
	CALL MPI_BCAST(ED,ND,MY_MPI_DP,IZERO,MPI_COMM_WORLD,IERR)
!
! 
!
...
!
	...
	1              POP_SPECIES(1,ISPEC),TA,ROOT(ID)%NXzV_F,IONE,ND,ND)
	        WRITE(172,'(2I5,3X,A12,3X,2ES14.4)')ID,ATM(ID)%NXzV_F,ION_ID(ID),ROOT(ID)%DXzV_F(1),ROOT(ID)%DXzV_F(ND)
	        FLUSH(UNIT=172)
	      END DO
	    END IF
	  END DO			!ISPEC
...
!
	...
	1         ATM(ID)%XzV_PRES)
	    ...
	    CALL MPI_BARRIER(MPI_COMM_WORLD,IERR)
            I=ND*NT
            CALL CHECK_VEC_NAN(POPS,I,'POPS after reduce in SET_NEW_MODELS',NAN_PRES)
!
! Compute the ion population at each depth.
! These are required when evaluation the occupation probabilities.
!
