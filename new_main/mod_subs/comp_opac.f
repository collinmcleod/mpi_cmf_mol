...
!
	  ...
	  IF(DST .EQ. 1)THEN
	    ...
	    IF(SECTION .EQ. 'CONTINUUM')THEN
	      IF(FREQ_INDX .EQ. 1)THEN
	         XRAY_LUM_TOT=0.0_LDP
	         XRAY_LUM_0P1=0.0_LDP
	         XRAY_LUM_1KEV=0.0_LDP
	      END IF
	      TA(DST:DEND)=ZETA(DST:DEND)*CLUMP_FAC(DST:DEND)*FQW(FREQ_INDX)
	      XRAY_LUM_TOT(DST:DEND)=XRAY_LUM_TOT(DST:DEND)+TA(DST:DEND)
	      IF(FL .GE. 100.0_LDP*T1)XRAY_LUM_0P1(DST:DEND)=XRAY_LUM_0P1(DST:DEND)+TA(DST:DEND)
	      IF(FL .GE. 1000.0_LDP*T1)XRAY_LUM_1KEV(DST:DEND)=XRAY_LUM_1KEV(DST:DEND)+TA(DST:DEND)
	    END IF
	  END IF
	ELSE
	  ETA_MECH=0.0_LDP
