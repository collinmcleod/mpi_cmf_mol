!
	SUBROUTINE INIT_OPAC_MOD(DST,DEND,ND,NT,DO_VAR_MATS)
	USE SET_KIND_MODULE
	USE OPAC_MOD
	IMPLICIT NONE
!
...
!
	...
	END IF
!
	IF(IOS .EQ. 0)ALLOCATE(dE_XRAY_0P1(ND),STAT=IOS)
	IF(IOS .EQ. 0)ALLOCATE(dE_XRAY_1KEV(ND),STAT=IOS)
	IF(IOS .EQ. 0)ALLOCATE(dE_XRAY_TOT(ND),STAT=IOS)
!
	IF(IOS .NE. 0)THEN
	  WRITE(LUER,*)'Error in INIT_OPAC_MOD'
	  ...
	  STOP
	END IF
	dE_XRAY_0P1=0.0_LDP; dE_XRAY_1KEV=0.0_LDP; dE_XRAY_TOT=0.0_LDP
!
	IF(DO_VAR_MATS)THEN
	  IF(IOS .EQ. 0)ALLOCATE(VCHI(NT,DST-1:DEND+1),STAT=IOS)              !Variation of CHI array.
          IF(IOS .EQ. 0)ALLOCATE(VETA(NT,DST-1:DEND+1),STAT=IOS)              !Variation of ETA array.
          IF(IOS .EQ. 0)ALLOCATE(VCHI_SAV(NT,DST-1:DEND+1),STAT=IOS)
          IF(IOS .EQ. 0)ALLOCATE(VETA_SAV(NT,DST-1:DEND+1),STAT=IOS)
          IF(IOS .EQ. 0)ALLOCATE(VCHI_ALL(NT,DST-1:DEND+1),STAT=IOS)
	  IF(IOS .EQ. 0)ALLOCATE(VCHI_ALL_SAV(NT,DST-1:DEND+1),STAT=IOS)
          IF(IOS .EQ. 0)ALLOCATE(VETA_ALL(NT,DST-1:DEND+1),STAT=IOS)
	  IF(IOS .EQ. 0)ALLOCATE(VETA_ALL_SAV(NT,DST-1:DEND+1),STAT=IOS)
	  IF(IOS .NE. 0)THEN
	    WRITE(LUER,*)'Error in INIT_OPAC_MOD'
