...
C
	     ...
	     IF(PLANE_PARALLEL_NO_V)THEN
	        IF(FIRST_FREQ .AND. J_IT_COUNTER .EQ. 0 .AND. MYPE .EQ. 0)WRITE(LUER,*)'Calling FCOMP_PP'
	...
	1               NBC_CMF,INBC,DBB,IC,THK_CONT,DIF,ND,NC,METHOD)
	     ELSE IF(PLANE_PARALLEL)THEN
	        IF(FIRST_FREQ .AND. J_IT_COUNTER .EQ. 0 .AND. MYPE .EQ. 0)WRITE(LUER,*)'Calling PP_FORM_CMF_V2'
...
!
	     ELSE IF(USE_FORMAL_REL)THEN
	       IF(FIRST_FREQ .AND. J_IT_COUNTER .EQ. 0 .AND. MYPE .EQ. 0)
...
!
	     ELSE
	       IF(FIRST_FREQ .AND. J_IT_COUNTER .EQ. 0. AND. MYPE .EQ. 0)
	1            WRITE(LUER,*)'Calling FG_J_CMF_MPI_V1 in COMP_J_BLANK'
