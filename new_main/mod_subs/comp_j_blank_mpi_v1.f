...
!
	SUBROUTINE COMP_J_BLANK_MPI_V1(
	1               SECTION,EDDINGTON,FL,FREQ_INDX,FIRST_FREQ,LST_ITERATION,MAXCH,
...
C
	...
	1            ESECEXT(1:NDEXT)*RJEXT_ES(1:NDEXT)
	     ...
	     IF(FIRST_FREQ .AND. J_IT_COUNTER .EQ. 0 .AND. MYPE .EQ. 0)THEN
	       WRITE(LUER,*)'Calling MOM_J_CMF_MPI_V1'; FLUSH(UNIT=LUER)
	     END IF
	     CALL MOM_J_CMF_MPI_V1(TA,CHIEXT,ESECEXT,VEXT,SIGMAEXT,REXT,
...
C
	  IF(COMPUTE_EDDFAC)THEN
	    ...
	    END IF
	  ELSE
	    READ(LU_EDD,REC=ACCESS_F,IOSTAT=IOS)(RJ(I),I=1,ND),T1
	    IF(IOS .NE. 0)THEN
	      WRITE(6,*)'Error reading EDDFACTOR file -- IOS, MYPE=',IOS,MYPE
	      WRITE(6,*)'ACCESS_F, FL=',ACCESS_F,FL
	      STOP
	    END IF
	    IF(ABS(T1/FL-1.0_LDP) .GT. 1.0E-10_LDP)THEN
	      WRITE(LUER,*)'Error - incorrect reading of EDDFACTOR in COMP_J_BLANK'
...
C
	     ...
	     IF(PLANE_PARALLEL_NO_V)THEN
	        IF(FIRST_FREQ .AND. J_IT_COUNTER .EQ. 0 .AND. MYPE .EQ. 0)THEN
	          WRITE(LUER,*)'Calling FCOMP_PP'; FLUSH(LUER)
	        END IF
	        SOURCE(1:ND)=TA(1:ND)/CHI_CLUMP(1:ND)
	        CALL FCOMP_PP_MPI_V1(R,TC,FEDD,SOURCE,CHI_CLUMP,IPLUS,HBC_CMF,
	1               NBC_CMF,INBC,DBB,IC,THK_CONT,DIF,ND,NC,METHOD)
	     ELSE IF(PLANE_PARALLEL)THEN
	        IF(FIRST_FREQ .AND. J_IT_COUNTER .EQ. 0 .AND. MYPE .EQ. 0)THEN
	          WRITE(LUER,*)'Calling PP_FORM_CMF_V2'; FLUSH(UNIT=LUER)
	        END IF
...
!
	     ELSE IF(USE_FORMAL_REL)THEN
	       IF(FIRST_FREQ .AND. J_IT_COUNTER .EQ. 0 .AND. MYPE .EQ. 0)THEN
	         WRITE(LUER,*)'Calling CMF_FORMAL_REL_V4 in COMP_J_BLANK'; FLUSH(UNIT=LUER)
	       END IF
...
!
	     ELSE
	       IF(FIRST_FREQ .AND. J_IT_COUNTER .EQ. 0. AND. MYPE .EQ. 0)THEN
	         WRITE(LUER,*)'Calling FG_J_CMF_MPI_V1 in COMP_J_BLANK'; FLUSH(UNIT=LUER)
	       END IF
	       CALL FG_J_CMF_MPI_V1(TA,CHI_CLUMP,CHI_SCAT_CLUMP,V,SIGMA,R,P,
	1                  TC,FEDD,INBC,HBC_CMF(1),IPLUS,FL,dLOG_NU,
	1                  INNER_BND_METH,DBB,IC,
	1                  VDOP_VEC,DELV_FRAC_FG,REXT_FAC,
...
C
	     ...
	     IF(PLANE_PARALLEL_NO_V)THEN
	       IF(FIRST_FREQ .AND. J_IT_COUNTER .EQ. 0 .AND. MYPE .EQ. 0)WRITE(LUER,*)'Calling MOM_J_PP_MPI_V1'
	       CALL MOM_J_PP_MPI_V1(TA,CHI_CLUMP,CHI_SCAT_CLUMP,
	...
	1                  IZERO,FIRST_FREQ,NEW_FREQ,ND)
	       ...
	       END IF
	     ELSE IF(PLANE_PARALLEL)THEN
	       IF(FIRST_FREQ .AND. J_IT_COUNTER .EQ. 0 .AND. MYPE .EQ. 0)WRITE(LUER,*)'Calling PP_MOM_CMF_V1'
	...
	1                  FIRST_FREQ,NEW_FREQ,ND)
	     ...
	     ELSE IF(USE_DJDT_RTE)THEN
	       IF(FIRST_FREQ .AND. J_IT_COUNTER .EQ. 0 .AND. MYPE .EQ. 0)WRITE(LUER,*)'Calling MOM_J_DDT_V6'
	...
	1              INCL_DJDT_TERMS,USE_DR4JDT,DJDT_RELAX_PARAM,NC,NP,ND,NCF)
	       HBC_CMF(1)=HFLUX_AT_OB/RJ(1)
	     ELSE IF(USE_J_REL)THEN
	       IF(FIRST_FREQ .AND. J_IT_COUNTER .EQ. 0 .AND. MYPE .EQ. 0)WRITE(LUER,*)'Calling MOM_JREL_V9'
	       CALL MOM_JREL_V9(TA,CHI_CLUMP,CHI_SCAT_CLUMP,V,SIGMA,R,
	...
	1             INCL_ADVEC_TERMS_IN_TRANS_EQ,INCL_REL_TERMS,FIRST_FREQ,ND)
	     ...
	     ELSE IF(USE_LAM_ES)THEN
	       IF(FIRST_FREQ .AND. J_IT_COUNTER .EQ. 0 .AND. MYPE .EQ. 0)THEN
	          WRITE(LUER,*)'Using USE_LAM_ES instead of solving moment equatons'
	       END IF
	       RJ(1:ND)=TC(1:ND)
	       ...
	       END IF
	     ELSE
	       IF(FIRST_FREQ .AND. J_IT_COUNTER .EQ. 0 .AND. MYPE .EQ. 0)WRITE(LUER,*)'Calling MOM_J_CMF_MPI_V1'
	       CALL MOM_J_CMF_MPI_V1(TA,CHI_CLUMP,CHI_SCAT_CLUMP,V,SIGMA,R,
