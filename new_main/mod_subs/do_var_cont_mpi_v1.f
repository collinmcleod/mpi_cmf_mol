	  ...
	  DO SIM_INDX=1,MAX_SIM
	    IF(.NOT. WEAK_LINE(SIM_INDX) .AND. RESONANCE_ZONE(SIM_INDX))THEN
	      ...
	      IF(NUM_BNDS .EQ. 3)THEN
	         K=ND
	         CALL MPI_ALLREDUCE(MPI_IN_PLACE,OPAC_FAC_MAT(:,SIM_INDX),K,MPI_DOUBLE_PRECISION,
	1               MPI_SUM,MPI_COMM_WORLD,IERR)
	         CALL MPI_ALLREDUCE(MPI_IN_PLACE,EMIS_FAC_MAT(:,SIM_INDX),K,MPI_DOUBLE_PRECISION,
	1               MPI_SUM,MPI_COMM_WORLD,IERR)
	         CALL MPI_ALLREDUCE(MPI_IN_PLACE,STIM_FAC_MAT(:,SIM_INDX),K,MPI_DOUBLE_PRECISION,
	1               MPI_SUM,MPI_COMM_WORLD,IERR)
	      END IF
	    END IF
	  END DO
