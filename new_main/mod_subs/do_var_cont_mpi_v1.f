...
!
	  ...
	  IF(PLANE_PARALLEL_NO_V)THEN
	    IF(FIRST_TIME .AND. MYPE .EQ. 0)THEN
	      WRITE(6,*)'Calling VAR_MOM_PP_MPI_V1'; FLUSH(UNIT=6)
	    END IF 
	...
	1           DIF,DBB,dDBBdT,dTdR,IC,METHOD,COHERENT_ES,DST,DEND,ND,NM)
	  ELSE IF(PLANE_PARALLEL)THEN
	    IF(FIRST_TIME .AND. MYPE .EQ. 0)THEN
	      WRITE(6,*)'Calling PP_VAR_MOM_CMF_V1'; FLUSH(UNIT=6)
	    END IF 
...
!
	    ...
	    IF(FIRST_TIME .AND. MYPE .EQ. 0)THEN
	      WRITE(6,*)'Calling VAR_JREL_MPI_V1'; FLUSH(UNIT=6)
	    END IF 
            CALL VAR_JREL_MPI_V1(TA,CHI_CLUMP,CHI_SCAT_CLUMP,ES_COH_VEC,V,SIGMA,R,
	1           dJ_DIF_d_T,dJ_DIF_d_dTdR,
	1           dRSQH_DIF_d_T,dRSQH_DIF_d_dTdR,WM,
...
!
	  ...
	  ELSE IF(USE_DJDT_RTE)THEN
	    IF(FIRST_TIME .AND. MYPE .EQ. 0)THEN
	      WRITE(6,*)'Calling VAR_MOM_J_DDT_MPI_V1'; FLUSH(UNIT=6)
	    END IF 
	    CALL VAR_MOM_J_DDT_MPI_V1(TA,CHI_CLUMP,CHI_SCAT_CLUMP,ES_COH_VEC,V,R,
	1           dJ_DIF_d_T,dJ_DIF_d_dTdR,dRSQH_DIF_d_T,dRSQH_DIF_d_dTdR,WM,FEDD,
	  ...
	  DO SIM_INDX=1,MAX_SIM
	    IF(.NOT. WEAK_LINE(SIM_INDX) .AND. RESONANCE_ZONE(SIM_INDX))THEN
	      ...
	      IF(NUM_BNDS .EQ. 3)THEN
	         K=ND
	         CALL MPI_ALLREDUCE(MPI_IN_PLACE,OPAC_FAC_MAT(:,SIM_INDX),K,MY_MPI_DP,
	1               MPI_SUM,MPI_COMM_WORLD,IERR)
	         CALL MPI_ALLREDUCE(MPI_IN_PLACE,EMIS_FAC_MAT(:,SIM_INDX),K,MY_MPI_DP,
	1               MPI_SUM,MPI_COMM_WORLD,IERR)
	         CALL MPI_ALLREDUCE(MPI_IN_PLACE,STIM_FAC_MAT(:,SIM_INDX),K,MY_MPI_DP,
	1               MPI_SUM,MPI_COMM_WORLD,IERR)
	      END IF
	    END IF
	  END DO
!	  IF(NUM_BNDS .EQ. 3)THEN
!	    K=3*ND*MAX_SIM
!	    CALL MPI_ALLREDUCE(MPI_IN_PLACE,FAC_MATS,K,MY_MPI_DP,MPI_SUM,MPI_COMM_WORLD,IERR!)
!	  END IF
...
!
	    ...
	    END DO
	    IF(NUM_BNDS .EQ. 3)THEN
	      CALL GATHER_FAC_MATS_V1(FAC_MATS,DST,DEND,ND,MAX_SIM)
!	      K=3*ND*MAX_SIM
!	      CALL MPI_ALLREDUCE(MPI_IN_PLACE,FAC_MATS,K,MY_MPI_DP,MPI_SUM,MPI_COMM_WORLD,IERR)
	    END IF
!
...
!
	  ...
	  CALL CHECK_VEC_NAN(TX,I,'TX(STOP)',NAN_PRES)
!
	  dJ_LOC=0.0_LDP
...
!
	  IF(DIF)THEN
	    ...
	    DO J=DIAG_INDX,NUM_BNDS
	      K=ND+DIAG_INDX-J
	      IF((DEND-K)*(K-DST) .GE. 0)THEN
	        DO I=1,NT-1
	          VJ(I,J,K)=VJ(I,J,K) + dJ_DIF_d_dTdR(K)*DIFFW(I)
	        END DO
	        VJ(NT,J,K)=VJ(NT,J,K) + dJ_DIF_d_T(K)+dJ_DIF_d_dTdR(K)*DIFFW(NT)
	      END IF 
	    END DO
	  END IF
...
!
	  DO K=DST,DEND		  	!Depth of intensity
