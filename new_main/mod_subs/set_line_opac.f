...
!
	...
	1            GLDGU(SIM_INDX)*U_STAR_RATIO(I,SIM_INDX)*POPS(NUP,I))
	    ...
	    IF(CHIL_MAT(I,SIM_INDX) .EQ. 0.0_LDP)THEN
	      IF(ZERO_CNT .EQ. 0)THEN
	        TB(DST:DEND)=0.0_LDP; TC(DST:DEND)=0.0_LDP
	      END IF
	      TB(I)=T1*L_STAR_RATIO(I,SIM_INDX)*POPS(NL,I)
	      TC(I)=T1*GLDGU(SIM_INDX)*U_STAR_RATIO(I,SIM_INDX)*POPS(NUP,I)
	...
	1                       'NL=',NL,'NUP=',NUP,'# depths=',ZERO_CNT,'Last depth=',LST_DEPTH
	    WRITE(LUER,'(1X,4(A,ES14.4,3X))')'OSCIL=',OSCIL(SIM_INDX),'Scale factor',T3,
	1                                    'Frequency',FL_SIM(SIM_INDX),'T1=',T1
	    WRITE(LUER,'(A10,10ES14.4,(10X,10ES14.4))')' POPS NL',(POPS(NL,J),J=DST,DEND)
	    WRITE(LUER,'(A10,10ES14.4,(10X,10ES14.4))')'POPS NUP',(POPS(NL,J),J=DST,DEND)
	    WRITE(LUER,'(A10,10ES14.4,(10X,10ES14.4))')'T1*L*PL',(TB(J),J=DST,DEND)
	    WRITE(LUER,'(A10,10ES14.4,(10X,10ES14.4))')'T1*G*U*NU',(TC(J),J=DST,DEND)
	    WRITE(LUER,'(A10,10ES14.4,(10X,10ES14.4))')'   L_STAR',(L_STAR_RATIO(J,SIM_INDX),J=DST,DEND)
	    WRITE(LUER,'(A10,10ES14.4,(10X,10ES14.4))')'   U_STAR',(U_STAR_RATIO(J,SIM_INDX),J=DST,DEND)
	    FLUSH(LUER)
	  END IF
!
