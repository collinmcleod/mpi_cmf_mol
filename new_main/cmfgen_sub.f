...
!
	...
	REAL(KIND=LDP) RAD_DECAY_LUM(DST:DEND)
	REAL(KIND=LDP) dE_WORK(DST:DEND)
	REAL(KIND=LDP) dE_SHOCK_POWER(DST:DEND)
	REAL(KIND=LDP) SHOCK_POWER_LUM(DST:DEND)
!
	LOGICAL FIRST
	LOGICAL CHK,SUCCESS
        LOGICAL VAR_SOB_JC
...
!
	IF(MYPE .EQ. 0)WRITE(6,'(A,/,T20,3(3X,A))')'Top of CMFGEN_SUB:','MYPE',' DST','DEND'
...
!
	...
	1            SOL_ABUND_REF_SET,L_FALSE,NUM_SPECIES,MYPE)
!
...
!
	  J=163+MYPE
	  I=DEND-DST+1
	  CALL WR2D(ATM(6)%XzV,ATM(6)%NXzV,I,'CIII',J); FLUSH(UNIT=J)
	  CALL WR2D(ATM(7)%XzV,ATM(7)%NXzV,I,'CIV',J); FLUSH(UNIT=J)
!
...
!
	  ...
	  END DO
	  J=155+MYPE
	  CALL WR2D(POPION,ND,IONE,'POPION',J); FLUSH(UNIT=J)
!
	  J=158+MYPE
	  CALL WR2D(POPS,NT,ND,'POPS',J); FLUSH(UNIT=J)
...
!
	  ...
	  CALL MPI_REDUCE(TB,TA,ND,MPI_DOUBLE_PRECISION,MPI_SUM,IZERO,MPI_COMM_WORLD,IERR)
	  IF(MYPE .EQ. 0)THEN
...
!
	    ...
	    WRITE(LU_POP,'(1X,1P8E16.7)')TB(1:ND)
!
