...
!
	DO ID=1,NUM_IONS-1
	  ID_SAV=ID
	  CALL SET_TWO_PHOT_MPI_V1(ION_ID(ID), ID_SAV,
	  ...
	  DO ID=1,NUM_IONS-1
	    IF(ATM(ID)%XzV_PRES .AND. COMPUTE_NEW_CROSS)THEN
	      DO J=1,ATM(ID)%N_XzV_PHOT
	       PHOT_ID=J
	       CALL QUAD_MULTI_MPI_V1(ATM(ID)%WSXzV(1,1,J), ATM(ID)%dWSXzVdT(1,1,J),
...
!
	    IF(LAST_ITERATIION)THEN
	      CALL MPI_REDUCE(MPI_IN_PLACE,AT_LEAT_ONE_NEG_OPAC,IONE,MPI_LOGICAL,IZERO,MPI_COMM_WORLD,IERR)
	      IF(LST_ITERATION .AND. AT_LEAST_ONE_NEG_OPAC)THEN
	        CALL MPI_REDUCE(NEG_OPACITY,AT_LEAT_ONE_NEG_OPAC,ND,MPI_LOGICAL,IZERO,MPI_COMM_WORLD,IERR)
	      END IF
	    END IF
!I
...
!
	    IF(ATM(ID)%XzV_PRES .AND. ATM(ID+1)%XzV_PRES .AND. XRAYS)THEN
	      CALL X_RRR_COOL_MPI_V1(X_RECOM(1,ATM(ID)%INDX_XzV),
...
! 
!
	CALL TWO_PHOT_RATE_MPI(T,RJ,FL,FQW(ML),DST,DEND,ND,NT)
!
! 
!
...
!
	      TA=0.0_LDP, TA(DST:DEND)=ZNET(DST:DEND,SIM_INDX)
	      CALL MPI_REDUCE(MPI_IN_PALCE,TA,ND,MPI_DOUBLE_PRECISION,MPI_SUM,IZERO,MPI_COMM_WORLD,IERR)
	      IF(MYPE .EQ. 0)WRITE(LU_NET,'(1P,5E14.6)')(TA(I),I=1,ND)
!
	      TB=0.0_LDP
	      DO I=DST,DEND
	         TB(I)=ZNET_SIM(I,SIM_INDX)*POPS(SIM_NUP(SIM_INDX),I)*U_STAR_RATIO(I,SIM_INDX)*EINA(SIM_INDX)
	      END DO
	      CALL MPI_REDUCE(MPI_IN_PALCE,TB,ND,MPI_DOUBLE_PRECISION,MPI_SUM,IZERO,MPI_COMM_WORLD,IERR)
...
!
	...
	1            AVE_ENERGY(SIM_NUP(SIM_INDX)))/VEC_FREQ(LS)
	      ...
	      TC=0.0_LDP
	      DO I=DST,DEND
	        TC=T3*ZNET_SIM(I,SIM_INDX)*ETAL_MAT(I,SIM_INDX)
	      END DO
	      CALL MPI_REDUCE(MPI_IN_PALCE,TC,ND,MPI_DOUBLE_PRECISION,MPI_SUM,IZERO,MPI_COMM_WORLD,IERR)
	      IF(MYPE .EQ. 0)WRITE(LU_HT,'(1X,1P,5E12.4)')(TC(I),I=1,ND)
!
...
!
	      ...
	      TB=0.0_LDP; TA(DST:DEND)=STEQ_T_NO_SCL(DST:DEND)
	      CALL MPI_REDUCE(MPI_IN_PALCE,TA,ND,MPI_DOUBLE_PRECISION,MPI_SUM,IZERO,MPI_COMM_WORLD,IERR)
	      CALL MPI_REDUCE(MPI_IN_PALCE,TB,ND,MPI_DOUBLE_PRECISION,MPI_SUM,IZERO,MPI_COMM_WORLD,IERR)
...
!
	DO ID=1,NUM_IONS-1
	  CALL EVAL_CHG_RATES_V3(ATM(ID)%CHG_PRXzV, ATM(ID)%CHG_RRXzV, ION_ID(ID),POPS,T,ND,NT)
	END DO
	IF(LST_ITERATION .AND. VERBOSE_OUTPUT)THEN
...
!
	IF(SN_MODEL .AND. DO_CO_MOV_DDT)THEN
          CALL STEQ_CO_MOV_DERIV_MPI_V3(POPS,ADVEC_RELAX_PARAM,LINEAR_ADV,
	...
	1             TIME_SEQ_NO,NUM_BNDS,ND,NT)
	  ...
	  END IF
	ELSE
	  WRITE(6,*)'Needs to be updated to USE POPS'
...
!	IF(DIAG_INDX .NE. 1)BA_T(NT,DIAG_INDX-1,ND)=-1.0D0
	IF(COMPUTE_BA .AND. WRBAMAT .AND. .NOT. FLUX_CAL_ONLY .AND. .NOT. LAMBDA_ITERATION)THEN
	  CALL TUNE(IONE,'STORE_BA')
	    CALL STORE_BA_DATA_MPI_V1(LU_BA,NION,NUM_BNDS,ND,COMPUTE_BA,FIXED_T,'BAMAT')
...
!
	IF(LST_ITERATION .AND. VERBOSE)THEN  ! .AND. .NOT. USE_FIXED_J)THEN
	  CALL WRITE_RECOM_MPI_V1(ND)
	  CALL WR_COOL_MPI_V1(ND)
...
!
	...
	1          .AND. .NOT. LAMBDA_ITERATION)THEN
	  ...
	  CALL STORE_BA_DATA_MPI_V13(LU_BA,NION,NUM_BNDS,ND,COMPUTE_BA,FIXED_T,'BAMAT')
