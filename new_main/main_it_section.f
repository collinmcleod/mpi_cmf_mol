...
!
	...
	1         TMP_STRING,NUM_BNDS,DST,DEND,ND,COMPUTE_BA)
	  ...
	  END DO
	  CALL TUNE(ITWO,'STEQ')
	  IF(MYPE .EQ. 0)WRITE(6,*)'Called STEQ routines',MYPE,DST,DEND
!
...
!
	...
	1        ID,TMP_STRING,OMEGA_GEN_V3,ED,T,DST,DEND,ND)
	      WRITE(6,*)ID,DST,ATM(ID)%COOLXzV(DST)
	      WRITE(6,*)ID,DEND,ATM(ID)%COOLXzV(DEND)
	    END IF
	  END DO
	  CALL TUNE(ITWO,'COL_DIGEST')
	  WRITE(6,*)'Called COLCOOL routines',MYPE,DST,DEND
!
	  IF(DST .EQ. 31)THEN
            WRITE(125,'(A,10ES14.4)')'C ',(ATM(12)%XzV(I,DST),I=4,11)
            WRITE(125,'(A,10ES14.4)')'C ',(ATM(12)%XzV(I,DST+1),I=4,11)
	    FLUSH(UNIT=125)
	  END IF
!
! 
...
40009	    FORMAT(1X,F5.0,2X,1P,2E12.4)
	...
	END DO		!End of Dielectronic section [do ML]
	  ...
	  WRITE(6,*)'Done dilectrinic section',MYPE,DST,DEND
! 
!***************************************************************************
!***************************************************************************
!
...
!
	...
	IF(DST .EQ. 1)THEN
	  WRITE(350,'(ES16.8)')(POPS(I,5), I=1,NT); FLUSH(UNIT=350)
	END IF
	IF(DST .GT. 40)THEN
	  WRITE(351,'(ES16.8)')(POPS(I,ND), I=1,NT);FLUSH(UNIT=351)
	END IF
!
	IF(DST .EQ. 31)THEN
          WRITE(125,'(A,10ES14.4)')'A ',(ATM(12)%XzV(I,DST),I=4,11)
          WRITE(125,'(A,10ES14.4)')'A ',(ATM(12)%XzV(I,DST+1),I=4,11)
	  FLUSH(UNIT=125)
        END IF
!
...
!
	...
	WRITE(6,*)'Starting 10000 loop'; FLUSH(6)
	DO 10000 ML=1,NCF
	  FREQ_INDX=ML
	  FL=NU(ML)
	  IF(ML .EQ. 1)THEN
	  ...
	  DO ID=1,NUM_IONS-1
	    IF(ATM(ID)%XzV_PRES)THEN
	      IF(ID .EQ. 12 .AND. DST .EQ. 31)THEN
	        WRITE(120,'(I8,ES14.4)')FREQ_INDX,FL
	        WRITE(120,'(10ES14.4)')(ATM(12)%XzV(I,DST),I=4,11)
	        WRITE(120,'(10ES14.4)')(ATM(12)%XzV(I,DST+1),I=4,11)
	      END IF
	      CALL PRRR_SL_MPI_V1(
...
!
	...
	IF(MOD(ML,1000) .EQ. 0 .AND. MYPE .EQ. 0)WRITE(6,*)'ML for the current loop is',ML,DST; FLUSH(UNIT=6)
!
! 
!
...
!
	...
	1             FIRST_OBS_COMP,NP_OBS)
	   END IF
!	   WRITE(700+MYPE,'(12ES14.4)')FL,HFLUX_AT_IB,HFLUX_AT_OB,RSQHNU(1:3),RSQHNU(ND-2:ND)
!	   FLUSH(UNIT=700+MYPE)
!
	ELSE
	  S1=(ETA(1)+RJ(1)*CHI_SCAT(1))/CHI(1)
...
!
	...
	WRITE(LUER,*)' '
!
	WRITE(6,*)'Col check after 1000 loop'
	  DO ID=1,NUM_IONS
	    IF(ATM(ID)%XzV_PRES)THEN
	      WRITE(6,*)ID,DST,ATM(ID)%COOLXzV(DST)
	      WRITE(6,*)ID,DEND,ATM(ID)%COOLXzV(DEND)
...
!
	...
	END IF
	WRITE(6,*)'Col check after CHG sect'
	  DO ID=1,NUM_IONS
	    IF(ATM(ID)%XzV_PRES)THEN
	      WRITE(6,*)ID,DST,ATM(ID)%COOLXzV(DST)
	      WRITE(6,*)ID,DEND,ATM(ID)%COOLXzV(DEND)
...
!
	...
	1            INCL_ADVECTION,LAMBDA_ITERATION,COMPUTE_BA)
	  ...
	  END IF
	END IF
	WRITE(6,*)'Col check after CM sect'
	  DO ID=1,NUM_IONS
	    IF(ATM(ID)%XzV_PRES)THEN
	      WRITE(6,*)ID,DST,ATM(ID)%COOLXzV(DST)
	      WRITE(6,*)ID,DEND,ATM(ID)%COOLXzV(DEND)
...
!
	...
	1                       DIAG_INDX,NUM_BNDS,NT,ND)
	  ...
	  END IF
	END IF
	WRITE(6,*)'Col check after eval temp'
	  DO ID=1,NUM_IONS
	    IF(ATM(ID)%XzV_PRES)THEN
	      WRITE(6,*)ID,DST,ATM(ID)%COOLXzV(DST)
	      WRITE(6,*)ID,DEND,ATM(ID)%COOLXzV(DEND)
...
!
	...
	END IF
	WRITE(6,*)'Col check after low heat'
	  DO ID=1,NUM_IONS
	    IF(ATM(ID)%XzV_PRES)THEN
	      WRITE(6,*)ID,DST,ATM(ID)%COOLXzV(DST)
	      WRITE(6,*)ID,DEND,ATM(ID)%COOLXzV(DEND)
...
!
	...
	END IF
!
	IF(MYPE .EQ. 0 .AND. LST_ITERATION .AND. .NOT. RD_FIX_T)THEN
!
...
!
	...
	WRITE(6,*)'Jumping all the rites -- needs to be fixed'
	GOTO 4242
!
!
...
!
	...
	END IF
!
	TA=LLUMST; CALL MPI_REDUCE(TA,LLUMST,ND,MPI_DOUBLE_PRECISION,MPI_SUM,IZERO,MPI_COMM_WORLD,IERR)
	TA=DIELUM; CALL MPI_REDUCE(TA,DIELUM,ND,MPI_DOUBLE_PRECISION,MPI_SUM,IZERO,MPI_COMM_WORLD,IERR)
	TA=DEP_RAD_EQ; CALL MPI_REDUCE(TA,DEP_RAD_EQ,ND,MPI_DOUBLE_PRECISION,MPI_SUM,IZERO,MPI_COMM_WORLD,IERR)
	TA=dE_WORK; CALL MPI_REDUCE(TA,dE_WORK,ND,MPI_DOUBLE_PRECISION,MPI_SUM,IZERO,MPI_COMM_WORLD,IERR)
	TA=RAD_DECAY_LUM; CALL MPI_REDUCE(TA,RAD_DECAY_LUM,ND,MPI_DOUBLE_PRECISION,MPI_SUM,IZERO,MPI_COMM_WORLD,IERR)
	TA=SHOCK_POWER; CALL MPI_REDUCE(TA,SHOCK_POWER,ND,MPI_DOUBLE_PRECISION,MPI_SUM,IZERO,MPI_COMM_WORLD,IERR)
	TA=XRAY_LUM_TOT; CALL MPI_REDUCE(TA,XRAY_LUM_TOT,ND,MPI_DOUBLE_PRECISION,MPI_SUM,IZERO,MPI_COMM_WORLD,IERR)
	TA=XRAY_LUM_0P1; CALL MPI_REDUCE(TA,XRAY_LUM_0P1,ND,MPI_DOUBLE_PRECISION,MPI_SUM,IZERO,MPI_COMM_WORLD,IERR)
	TA=XRAY_LUM_1KeV; CALL MPI_REDUCE(TA,XRAY_LUM_1KeV,ND,MPI_DOUBLE_PRECISION,MPI_SUM,IZERO,MPI_COMM_WORLD,IERR)
	TA=MECH_LUM; CALL MPI_REDUCE(TA,MECH_LUM,ND,MPI_DOUBLE_PRECISION,MPI_SUM,IZERO,MPI_COMM_WORLD,IERR)
	TA=DJDT_FLUX; CALL MPI_REDUCE(TA,DJDT_FLUX,ND,MPI_DOUBLE_PRECISION,MPI_SUM,IZERO,MPI_COMM_WORLD,IERR)
!
	IF(MYPE .EQ. 0)THEN
...
!
	  ...
	  ELSE
	    DO I=ND-1,1,-1
	      T3=T3+MECH_LUM(I)+DJDT_FLUX(I)+dE_WORK(I)-RAD_DECAY_LUM(I)-SHOCK_POWER_LUM(I)
...
!
	IF(.NOT. COMPUTE_BA)THEN
	  CALL READ_BA_DATA_MPI_V1(LU_BA,NION,NUM_BNDS,ND,COMPUTE_BA,FIXED_T,SUCCESS,'BAMAT')
	  IF(.NOT. SUCCESS)THEN
...
4242	CONTINUE
	...
	I=660+MYPE
        CALL WR2D_V2(POPS,NT,ND,'POPS ARRAY','#',L_TRUE,I)
