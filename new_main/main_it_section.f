...
!
	  IF(LST_ITERATION .AND. VERBOSE_OUTPUT)THEN
	    IF(MYPE .EQ. 0)THEN
	      ...
	      CALL GET_LU(K,'WR_ASCI_STEQ_MPI in main_it_section')
	      OPEN(UNIT=K,STATUS='UNKNOWN',ACTION='WRITE',FILE='COLLISIONAL_TERMS')
	    FLUSH(UNIT=6)
	    END IF
	    CALL MPI_BARRIER(MPI_COMM_WORLD,IERR)
	    CALL WR_ASCI_STEQ_MPI_V1(NION,DST,DEND,ND,'STEQ ARRAY- Collisional Terms',K)
	    IF(MYPE .EQ. 0)THEN
	      CLOSE(K)
