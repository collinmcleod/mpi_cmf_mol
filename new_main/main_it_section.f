...
20000	CONTINUE
	...
	IF(MYPE .EQ. 0)THEN
	   WRITE(6,*)'Start of GIT loop -- MYPE =',MYPE
	   FLUSH(UNIT=6)
	END IF
!
	IF(MYPE .EQ. 0)THEN
...
!
	...
	SECTION='DTDR'
	IF(1  .EQ. 1)THEN
!IMPURITY_CODE .OR. USE_FIXED_J .OR. FLUX_CAL_ONLY .OR. (RD_LAMBDA .AND. NEWMOD .AND. .NOT. SN_MODEL))THEN
...
!
	  ...
	  END IF
	  I=NTHREAD-1
	  WRITE(6,*)'Befor BARRIER , MYPE=',MYPE; FLUSH(UNIT=6)
	  CALL MPI_BARRIER(MPI_COMM_WORLD,IERR)
!
...
!
	...
	IF(MYPE .EQ. 0)THEN
	  WRITE(LUER,'(/,1X,A,ES13.6)')'The value of DTDR is:',DTDR
...
!
	...
	CALL MPI_BARRIER(MPI_COMM_WORLD,IERR)
	DO 10000 ML=1,NCF
	  IF(MOD(ML,50000) .EQ. 0 .AND. MYPE .EQ. 0)THEN
	    WRITE(6,*)'MYPE,ML=',MYPE,ML
	    FLUSH(UNIT=6)
	  END IF
	  FREQ_INDX=ML
	  FL=NU(ML)
...
!
	...
	FLUSH(UNIT=200+MYPE)
!
	WRITE(6,*)'About to gather LLUMST'; FLUSH(UNIT=6)
...
!
	WRITE(6,*)'About to do LLUMST lum computation'; FLUSH(UNIT=6)
	IF(MYPE .EQ. 0)THEN
