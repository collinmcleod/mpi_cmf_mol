...
!
	...
	WRITE(6,*)'Starting 10000 loop', WRITE_JH; FLUSH(6)
	DO 10000 ML=1,NCF
	  FREQ_INDX=ML
	  FL=NU(ML)
	  IF(ML .EQ. 1)THEN
...
!	IF(MOD(ML,5000) .EQ. 0 .AND. MYPE .EQ. 0)WRITE(6,*)'ML for the current loop is',ML,DST; FLUSH(UNIT=6)
!
! 
!
...
!
	...
	END IF
!
	CALL WRITE_VEC(DEP_RAD_EQ_LUM,ND,'DEP_RAD_EQ_LUM',200+MYPE)
	CALL WRITE_VEC(DJDT_LUM,ND,'DJDT_LUM',200+MYPE)
	CALL WRITE_VEC(SHOCK_POWER_LUM,ND,'SHOCK_POWER_LUM',200+MYPE)
	CALL WRITE_VEC(RAD_DECAY_LUM,ND,'RAD_DECAY_LUM',200+MYPE)
	CALL WRITE_VEC(XRAY_LUM_TOT,ND,'XRAY_LUM_TOT',200+MYPE)
	FLUSH(UNIT=200+MYPE)
!
	WRITE(6,*)'About to gather LLUMST'
