...
!
! Do the same as above for the molecular reactions
!
	IF (INCL_MOL_RXN) THEN
	   CALL SET_MOL_RXN_LEV_MPI(ND,LUMOD)
	   CALL VERIFY_MOL_RXN()
...
!
	...
	IF(.NOT. NEWMOD)THEN
	  ...
	  ELSE IF(MYPE .EQ. 0 .AND. SUCCESS)THEN
	    WRITE(LUER,'(A,/)')' Successfully opened/read all BAMAT files in CMFGEN_SUB'
	  END IF
	END IF
...
!
! We no initialize TUNE for each iteration -- this makes it easier to see how much
! time each tpe of iteration is taking. We will also initialize it after computing DTDR.
!
20000	CONTINUE
	CALL TUNE(IZERO,' ')
	CALL TUNE(IONE,'GIT')
...
!
	DO I=0,NTHREAD-1
	  IF(MYPE .EQ. 0 .AND. I .EQ. MYPE)THEN
	    ...
	    WRITE(LUER,'(109A10)')'MYPE','IT_COUNT','IREC','RD_LAM','LAMBDA','FIXED_T',
	1                       'COMP._BA','COH._ES','SN_MODEL','Max Mem'
	  END IF
	  IF(MYPE .EQ. I)THEN
	    CALL GET_MAX_MEMORY(T1,T2,MEM_UNIT,' ',IZERO)
	    WRITE(LUER,'(3I10,6(9X,L1),F10.3,A)')MYPE,MAIN_COUNTER,IREC,RD_LAMBDA,LAMBDA_ITERATION,
	1                   FIXED_T,COMPUTE_BA,COHERENT_ES,SN_MODEL,T2,MEM_UNIT
...
!
	  ...
	  CALL MPI_BCAST(DTDR,IONE,MY_MPI_DP,I,MPI_COMM_WORLD,IERR)
	END IF
	CALL TUNE(ITWO,'DTDR')
!
...
!
	...
	1         LOC_ID,TMP_STRING,OMEGA_GEN_V4,
	1         ATM(ID)%EQXzV,NUM_BNDS,DST,DEND,ND,NION,NT,
	1         COMPUTE_BA,FIXED_T,LST_ITERATION,IS_MOLECULE(SPECIES_LNK(ID)))
!
...
!
	...
	1         TMP_STRING,NUM_BNDS,DST,DEND,ND,COMPUTE_BA)
	  ...
	  CALL TUNE(ITWO,'STEQ')
	  CALL MPI_BARRIER(MPI_COMM_WORLD,IERR)
	  IF(MYPE .EQ. 0)WRITE(6,*)'Called both STEQ routines',MYPE,DST,DEND
!
...
!
	  IF(LST_ITERATION .AND. VERBOSE_OUTPUT)THEN
	    IF(MYPE .EQ. 0)THEN
	      WRITE(6,*)'Calling WR_ASCI_STEQ_MPI'
	      OPEN(UNIT=LU_DR,STATUS='UNKNOWN',ACTION='WRITE',FILE='COLLISIONAL_TERMS')
	    FLUSH(UNIT=6)
	    END IF
	    CALL MPI_BARRIER(MPI_COMM_WORLD,IERR)
	    ...
	    IF(MYPE .EQ. 0)THEN
	      CLOSE(LU_DR)
	      WRITE(6,*)'Called WR_ASCI_STEQ_MPI'; FLUSH(UNIT=6)
	    END IF
	  END IF
!
	  IF(MYPE .EQ. 0)WRITE(6,*)'CHECK_EHB_BALANCE open', MYPE; FLUSH(UNIT=6)
...
!
	...
	1        ID,TMP_STRING,OMEGA_GEN_V4,ED,T,DST,DEND,ND,IS_MOLECULE(SPECIES_LNK(ID)))
	    END IF
	  END DO
...
!
	...
	IF(ACCURATE .OR. EDDINGTON)THEN
	  ...
	  END IF
	  CALL MPI_BCAST(ACCESS_F,IONE,MPI_INTEGER,IZERO,MPI_COMM_WORLD,IERR)
	END IF
!
	DO ML=1,NDIETOT
...
!
	...
	DO I=0,NTHREAD-1
	  IF(I .EQ. 0 .AND. MYPE .EQ. I)THEN
	    FLUSH(UNIT=6) 
	    WRITE(6,'(/,A,/)')' Checking access parameters for EDDINGTION file.'
...
!
	      ...
	      CALL MPI_REDUCE(TA,TB,ND,MY_MPI_DP,MPI_SUM,IZERO,MPI_COMM_WORLD,IERR)
	      IF(MYPE .EQ. 0)THEN
...
!
! Sum cooling rates from carbon monoxide LTE line emission
!
	IF (.NOT. ALLOCATED(COMI_NET_COOLING)) THEN
	   ALLOCATE(COMI_NET_COOLING(DST:DEND))
	   COMI_NET_COOLING(:)=0.0_LDP
	END IF
	IF (INCL_MOL_RXN) THEN
	   IF (ML .EQ. 1) COMI_NET_COOLING(:)=0.0_LDP
	   DO SIM_INDX=1,MAX_SIM
	      IF(END_RES_ZONE(SIM_INDX))THEN
		 LS=SIM_LINE_POINTER(SIM_INDX)
		 IF (INDEX(TRANS_NAME_SIM(SIM_INDX),'COMI') .NE. 0) THEN
		    IF(SCL_LINE_COOL_RATES .OR. SCL_SL_LINE_OPAC)THEN
		       SCL_FAC=(AVE_ENERGY(SIM_NL(SIM_INDX))-
	1		    AVE_ENERGY(SIM_NUP(SIM_INDX)))/VEC_FREQ(LS)
		       IF(ABS(SCL_FAC-1.0_LDP) .GT. SCL_LINE_HT_FAC)SCL_FAC=1.0_LDP
		    ...
		    END IF
		    T3=SCL_FAC; IF(SCL_SL_LINE_OPAC)T3=1.0_LDP
		    T3 = T3*1.0E-10_LDP*16.0_LDP*ATAN(1.0_LDP) ! 4 pi / 10^10 (solid angle and unit convention)
		    DO I=DST,DEND
		       COMI_NET_COOLING(I)=COMI_NET_COOLING(I)+T3*ZNET_SIM(I,SIM_INDX)*ETAL_MAT(I,SIM_INDX)
		    END DO
		 END IF
	      END IF
	   END DO
...
!
	DO ID=1,NUM_IONS-1
	  ID_SAV=ID
	  IF(ATM(ID)%XzV_PRES)THEN
	    CALL SET_CHG_EXCH_MPI_V1(ID_SAV, ATM(ID)%XzVLEVNAME_F,
	1       ATM(ID)%EDGEXzV_F,  ATM(ID)%GXzV_F,
	1       ATM(ID)%F_TO_S_XzV, ATM(ID)%GIONXzV_F,
	1       ATM(ID)%NXzV_F, ATM(ID)%NXzV, DST, DEND, ND,
...
!
	...
	CALL MPI_BARRIER(MPI_COMM_WORLD,IERR)
!
! Include molecular reaction rates
!
	IF (INCL_MOL_RXN) THEN
	   CALL TUNE(IONE,'MOL_RXN')
	   WRITE(*,*) 'PROC ',MYPE,' CALLING STEQ_BA_MOL_RXN'
	   CALL FLUSH(6)
	   CALL STEQ_BA_MOL_RXN_MPI(POPS,NT,ND,DIAG_INDX,COMPUTE_BA)
	   IF (LST_ITERATION .AND. VERBOSE_OUTPUT) THEN
	      WRITE(199,'(I10,2ES18.8,3X,A)')ML,STEQ_T(DPTH_INDX),BA_T(VAR_INDX,DIAG_INDX,DPTH_INDX),'Mol_rxn'
	   END IF
	   CALL TUNE(ITWO,'MOL_RXN')
	   CALL MPI_BARRIER(MPI_COMM_WORLD,IERR)
	END IF
!
! Penning
...
!
	IF(SN_MODEL .AND. DO_CO_MOV_DDT)THEN
	  CALL EVAL_TEMP_DDT_MPI_V1(dE_WORK,AD_COOL_V,AD_COOL_DT,
	...
	1                       DIAG_INDX,NUM_BNDS,NT,ND)
	  IF(LST_ITERATION .AND. VERBOSE_OUTPUT)THEN
	    ...
	    CALL WR2D_MPI_V1(TA(DST:DEND),IONE,DST,DEND,ND,'Adiabatic term','&',L_TRUE,LU_T_EHB)
	  END IF
	END IF
	CALL MPI_BARRIER(MPI_COMM_WORLD,IERR)
!
...
!
	...
	END DO
!
	TA=ROSS_MEAN;   CALL MPI_ALLREDUCE(TA,ROSS_MEAN,ND,MY_MPI_DP,MPI_SUM,MPI_COMM_WORLD,IERR)
	TA=ABS_MEAN;    CALL MPI_ALLREDUCE(TA,ABS_MEAN,ND,MY_MPI_DP,MPI_SUM,MPI_COMM_WORLD,IERR)
	TA=FLUX_MEAN;   CALL MPI_ALLREDUCE(TA,FLUX_MEAN,ND,MY_MPI_DP,MPI_SUM,MPI_COMM_WORLD,IERR)
	TA=PLANCK_MEAN; CALL MPI_ALLREDUCE(TA,PLANCK_MEAN,ND,MY_MPI_DP,MPI_SUM,MPI_COMM_WORLD,IERR)
	TA=INT_dBdT;    CALL MPI_ALLREDUCE(TA,INT_dBdT,ND,MY_MPI_DP,MPI_SUM,MPI_COMM_WORLD,IERR)
...
!
	  ...
	  END IF
!
	  IF(MYPE .EQ. 0 .AND. CHK .AND. COMP_GREY_LST_IT .AND. .NOT. USE_FIXED_J)THEN
...
!
	...
	IF(DO_HYDRO)THEN
	  K=MAIN_COUNTER
	  CALL DO_CMF_HYDRO_MPI_V1(POPS,LUM,TEFF,LOGG,STARS_MASS,RP,RMAX,RMDOT,VINF,V_BETA1,
	1          PRESSURE_VTURB,PLANE_PARALLEL,PLANE_PARALLEL_NO_V,
	1          K,DONE_HYDRO_REVISION,NC,ND,NP,NT)
	END IF
!
! CHK is a temporary variable used to indcate that we have changed the velocity law.
! It used in the following if statement.
!
