...
!
	...
	1           ES_J, ES_NU, ES_V, ES_ND, ES_NCF, NC_ATM)
	USE SET_KIND_MODULE
...
!
	...
	DO I=3,ND-4
	  T1=MIN(T1,V(I)-V(I+1))
	END DO
	WRITE(6,'(/,A,F10.2,3X,A,ES14.4,/)')' Minimum step (excluding near boundaries) size in V is ',T1,'V(1)=',V(1)
	...
	BETA_LAW='UNIFORM_BETA';  CALL GEN_IN(BETA_LAW,'Beta distribution - UNIFORM_BETA, UNFORM_COSB, POWER, BMIN')
	TOP_BOT_SYM=.TRUE.;       CALL GEN_IN(TOP_BOT_SYM,'Top-bottom symmetric?')
	LAM_START=4000.0_LDP;     CALL GEN_IN(LAM_START, 'Minimum wavelength of 2D grid (Ang)')
...
!
	...
	IF(BETA_LAW .EQ. 'POWER')THEN
	  PARAMS(1)=1.5;	  CALL GEN_IN(PARAMS(1),'Parameter 1 for BETA_LAW=POWER')
	ELSE IF(BETA_LAW .EQ. 'BMIN')THEN
	  NPAR=2
	  PARAMS(1)=3;	          CALL GEN_IN(PARAMS(1),'Parameter 1 for BETA_LAW=BMIN')
	  PARAMS(2)=1.5;	  CALL GEN_IN(PARAMS(2),'Parameter 2 for BETA_LAW=BMIN')
	END IF
!
        CALL CREATE_SHIFTED_SN_R_GRID(R,V,dV,ND,NB)
        CALL COMPUTE_BETA(BETA_LAW,PARAMS,NPAR,TOP_BOT_SYM,NB)
...
CONTAINS
	SUBROUTINE COMPUTE_BETA(BETA_LAW,PARAMS,NPAR,TOP_BOT_SYM,NB)
	USE SET_KIND_MODULE
	IMPLICIT NONE
!
...
!
	...
	dBETA_MIN=100.0D0
	dBETA_MAX=-100.0D0
	WRITE(LU_OUT,'(A7,2A14)')'Index','Beta','dBETA'
	DO I=1,NB_MULTI-1
