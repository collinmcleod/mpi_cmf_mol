	...
	IMPLICIT NONE
!
! Altered 16-Aug-2024 : BAsed on se_ba_non_therm_v3.f90
!
        INTEGER NT
        INTEGER ND
!
...
!
	    DO IPROC=1,NUM_DEPTHS_PER_THREAD
	      DPTH_INDX=GET_DEPTH_INDX(MYPE,IPROC,MAX_NUM_THREADS_PER_CORE) 
!
...
!
!
	DO IPROC=1,NUM_THREADS_PER_CORE
          DPTH_INDX=GET_DEPTH_INDX(MYPE,IPROC,MAX_NUM_THREADS_PER_CORE) 
	  DO I=1,NKT
...
!
	WRK_VEC_A=0.0_LDP;  WRK_VEC_B=0.0D0
	DO IPROC=1,NUM_THREADS_PER_CORE
          DPTH_INDX=GET_DEPTH_INDX(MYPE,IPROC,MAX_NUM_THREADS_PER_CORE) 
	  T1=0.0_LDP
	  T2=0.0_LDP
	  ...
	  END DO
	  WRK_VEC_A(DPTH_INDX)=T1*SQRT(0.5_LDP*9.109389E-28_LDP/1.602177E-12_LDP)*WRK_EDEP_eV(DPTH_INDX)
	  WRK_VEC_B(DPTH_INDX)=T2**SQRT(0.5_LDP*9.109389E-28_LDP/1.602177E-12_LDP)*WRK_EDEP_eV(DPTH_INDX)
	END DO
	CALL MPI_REDUCE(WRK_VEC_A,A_SUM,ND,MPI_DOUBLE_PRECISION,MPI_SUM,IZERO,MPI_COMM_WORLD,IERR)
	CALL MPI_REDUCE(WRK_VEC_B,B_SUM,ND,MPI_DOUBLE_PRECISION,MPI_SUM,IZERO,MPI_COMM_WORLD,IERR)
!
	WRITE(LU_TH,*)''
	WRITE(LU_TH,'(X,A5,6(A12))')'Depth','Ne(NT)','Ne','Ne(NT)/Ne','Ek(NT)','Ek','Ek(NT)/Ek'
	DO DPTH_INDX=1,ND
	  WRITE(LU_TH,'(X,I5,6ES12.4)')DPTH_INDX,T1,ED(DPTH_INDX),A_SUM(DPTH_INDX)/ED(DPTH_INDX),             &
	                             B_SUM(DPTH_INDX),1.5_LDP*8.617343E-5_LDP*ED(DPTH_INDX)*T(DPTH_INDX)*1.0E4_LDP,   &
	                             B_SUM(DPTH_INDX)/(1.5_LDP*8.617343E-5_LDP*ED(DPTH_INDX)*T(DPTH_INDX)*1.0E4_LDP)
	END DO
!
	...
	WRITE(LU_TH,'(//,A,9(3X,A))')       &
	            ...
	            '  E_ion  ','  E_exc  ','  E_cool '
!
	WRK_VEC_A=0.0_LDP; WRK_VEC_B=0.0_LDP
	DO IPROC=1,NUM_THREADS_PER_CORE
	  DPTH_INDX=GET_DEPTH_INDX(MYPE,IPROC,MAX_NUM_THREADS_PER_CORE) 
	  IF(DPTH_INDX .GT. ND)EXIT
	  IF(WRK_EDEP_eV(I) .NE. 0.0_LDP)THEN
	    WRK_VEC_A(DPTH_INDX)=Hz_TO_eV*LOCAL_ION_HEATING(I)/WRK_EDEP_eV(I)
	    WRK_VEC_B(DPTH_INDX)=Hz_TO_eV*LOCAL_EXC_HEATING(I)/WRK_EDEP_eV(I)
	  ELSE
	    T2=0.0_LDP
	    T3=0.0_LDP
	  END IF
...
!
	DO DPTH_INDX=1,ND
	  I=DPTH_INDX
	  T1=(1.0_LDP-WRK_VEC_A(I)-WRK_VEC_B(I))
