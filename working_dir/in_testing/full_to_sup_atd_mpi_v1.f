...
C            SUPER levels).
C
	SUBROUTINE FULL_TO_SUP_ATM_MPI_V1(
...
C
	INTEGER DST,DEND
	REAL(KIND=LDP) DC2_S(DST:DEND)		!Ion density (Super levels)
	REAL(KIND=LDP) DC2_F(DST:DEND)		!Ion density (Full model atom)
C
	INTEGER NC2_F
	REAL(KIND=LDP) C2_F(NC2_F,DST:END)
	INTEGER F_TO_S_MAP_C2(NC2_F)
C
	INTEGER NC2_S
	REAL(KIND=LDP) C2_S(NC2_S,DST:DENDD)
	LOGICAL C2_PRES
C
	INTEGER NCIII_S
	REAL(KIND=LDP) CIII_S(NCIII_S,ND)
	LOGICAL CIII_PRES
C
C Local variables.
C
