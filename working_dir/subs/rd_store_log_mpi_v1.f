!
! Aletere: 18-Aug-2024:  Added calls to MPI_ABORT.
...
!	          CALL RD_OPTIONS_INTO_STORE(LU_IN,LUO)
!             and end with
!	          CALL CLEAN_RD_STORE
!
	MODULE RD_VAR_MOD
...
!
	...
	IF(STRING(1:1) .NE. 'T' .AND. STRING(1:1) .NE. 'F')THEN
	  ...
	  STOP
	END IF
	IF(MYPE .EQ. 0)WRITE(LUO,10)VALUE,TRIM(KEY),TRIM(A)
...
!
	...
	IF(IOS .NE. 0)THEN
	  WRITE(LUER,*)'Error reading 2 logical values with RD_STORE_LOG (RD_STORE_2LOG).'
	  WRITE(LUER,*)'KEY is ',TRIM(KEY)
	  WRITE(LUER,*)'String with error follows:'
	  WRITE(LUER,*)TRIM(STRING)
	  CALL MY_MPI_ABORT()
	  STOP
	END IF
	IF(MYPE .EQ. 0)WRITE(LUO,10)VALUE1,VALUE2,TRIM(KEY),TRIM(A)
...
!
	...
	DO WHILE(I.LE. LEN_TRIM(STRING))
	  ...
	  END IF
	END DO
	IF(MYPE .EQ. 0)WRITE(LUO,10)TRIM(STRING),TRIM(KEY),TRIM(A)
...
!
	...
	IF( ABS(VALUE1) .LT. 100.0_LDP .AND. ABS(VALUE2) .LT. 100.00_LDP)THEN
	  WRITE(STRING,'(F8.4,A,F8.4)')VALUE1,',',VALUE2
	ELSE
	  WRITE(STRING,'(F12.4,A,F12.4)')VALUE1,',',VALUE2
	END IF
	I=1
	DO WHILE(I.LE. LEN_TRIM(STRING))
	  IF(STRING(I:I) .EQ. ' ')THEN
...
!
	...
	IF(J .LE. 13)THEN
	  ...
	  IF(MYPE .EQ. 0)WRITE(LUO,10)TMP_STR,TRIM(KEY),TRIM(A)
10	  FORMAT(A13,5X,'[',A,']',T40,A)
	ELSE IF(J+7+LEN_TRIM(KEY) .LT. 35)THEN
	  IF(MYPE .EQ. 0)WRITE(LUO,11)TRIM(VALUE),TRIM(KEY),TRIM(A)
11	  FORMAT(/,A,5X,'[',A,']',T40,A,/)
	ELSE
	  IF(MYPE .EQ. 0)WRITE(LUO,12)TRIM(VALUE),TRIM(KEY),TRIM(A)
...
!
	...
	END IF
	I=LEN_TRIM(STRING)
	IF(MYPE .EQ. 0)WRITE(LUO,10)STRING(1:MAX(12,I)),TRIM(KEY),TRIM(A)
...
!
	CALL GET_KEY_STRING(STRING,KEY,MUST_BE_PRES,KEY_FOUND)
	IF(.NOT. KEY_FOUND)RETURN
	READ(STRING,*,IOSTAT=IOS)VALUE1,VALUE2,VALUE3
	IF(IOS .NE. 0)THEN
