...
!
	I=N_SET_UP_VECS*ND
	CALL MPI_ALLREDUCE(MPI_IN_PLACE,SET_UP_VECS,I,MY_MPI_DP,MPI_SUM,MPI_COMM_WORLD,IERR)
!
...
!
	I=4*VEC_LENGTH
	CALL MPI_ALLREDUCE(MPI_IN_PLACE,TRI_VECS,I,MY_MPI_DP,MPI_SUM,MPI_COMM_WORLD,IERR)
!	IF(MYPE .EQ. 0)THEN
!	  DO I=1,ND
...
!	IF(MOD(COUNTER,10000) .EQ. 0)THEN
	IF(MOD(COUNTER,1000000000) .EQ. 0)THEN
