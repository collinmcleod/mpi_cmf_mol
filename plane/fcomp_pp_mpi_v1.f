...
!
	K=(ND-1)/NTHREAD+1
	DST=1+MYPE*K; DEND=MIN(MYPE*K,ND)
	dCHIdR=0.0_LDP
...
!
	    ...
	    PAR_INBCNEW=INBCNEW + JQW(LS)*MU(LS)*(XM(ND)-(XM(ND)-XM(ND-1))/DTAU(ND-1))
	  ...
	  ELSE
	    ...
	    PAR_HBC_S=PAR_HBC_S+JQW(LS)*IBOUND*MU(LS)
	    PAR_INBCNEW=INBCNEW + JQW(LS)*MU(LS)*(SOURCE(ND)-(SOURCE(ND)-SOURCE(ND-1))/DTAU(ND-1))
	    IPLUS(LS)=0.0_LDP
	  END IF
!
2000	CONTINUE
