...
!
	...
	INTEGER MYPE,NTHREAD,NUM_DEPTHS_PER_THREAD
	INTEGER IERR
	INCLUDE 'mpif.h'
!
!
...
!
	...
	DEND=(MYPE+1)**NUM_DEPTHS_PER_THREAD
	DEND=MIN(DEND,ND)
!	
	IF(ND .GT. ND_SM)THEN
	  ...
	  NEW_DATA(DST:DEND,3)=EXP(NEW_DATA(DST:DEND,2))
...
!
	NUM_DEPTHS_PER_THREAD=(ND-1)/NTHREAD+1
	DST=MYPE*NUM_DEPTHS_PER_THREAD+1
	DEND=MIN(ND,(MYPE+1)*NUM_DEPTHS_PER_THREAD)
	M_DST=MAX(1,DST-1)
...
!
	I=4*ND
	CALL MPI_ALLREDUCE(MPI_IN_PLACE,THOM_VEC,I,MPI_DOUBLE_PRECISION,MPI_COMM_WORLD,IERR)
