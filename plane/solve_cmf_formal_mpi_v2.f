!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
!
      SUBROUTINE SOLVE_CMF_FORMAL_MPI_V2(CHI,ETA,IP,FREQ,NU_DNU,
     *                     INNER_BND_METH,B_NUE,dBDTAU,
     *                     RSQH_IB_PNT_SRCE,R_PNT_SRCE,ND,NP,NC,NC_PNT)
      USE SET_KIND_MODULE
      USE MOD_SPACE_GRID_MPI_V1
      IMPLICIT NONE
!
! Altered: 10-Feb-2026 : Changed to V2
!                        RSQH_IB_PNT_SRCE, R_PBT_SRCE, and NC_PNT added to call.
...
!
      ...
      R_IB=R_EXT_SAV(ND_EXT_SAV)
      RSQH_IB_PNT_SRCE=0.0_LDP
      IF(NC_PNT .NE. 0)THEN
	CALL SET_PNT_SOURCE_RAD(I_PNT_SRCE,RSQH_IB_PNT_SRCE,FREQ,R_PNT_SRCE,R_IB)
      END IF
!
!---------------------------------------------------------------
!
...
!
      if(ip .le. nc_pnt)then
        ray(ip)%I_p(nzz)=ray(ip)%I_m(nzz)+I_PNT_SRCE
        write(701,*)FREQ,ray(ip)%I_m(nzz),I_PNT_SRCE
      else if(ip .gt. nc .or. INNER_BND_METH .eq. 'ZERO_FLUX')then
