...
!
	END MODULE EXT_REL_GRID_MPI_V2
!
	SUBROUTINE CMF_FORMAL_REL_MPI_V2
	1           (ETA,CHI,ESEC,V,SIGMA,R,P,
	1            JNU,FEDD,RET_HNU_AT_IB,RET_HNU_AT_OB,IPLUS,
	1            FREQ,dLOG_NU,B_PLANCK,DBB,
	1            INNER_BND_METH,THICK_OB,PNT_SRCE_MOD,
	1            VDOP_VEC,VDOP_FRAC,REXT_FAC,
	1            DUST_SCAT_OPAC,G_HEN_GREEN,USE_HEN_GREEN,
	1            METHOD,INITIALIZE,NEW_FREQ,
	1            R_PNT_SRCE,NC_PNT,NC,NP,DST,DEND,ND)
	USE SET_KIND_MODULE
        USE EXT_REL_GRID_MPI_V2
	USE MOD_SPACE_GRID_MPI_V1
	USE MOD_RAY_MOM_STORE
	USE MPI
	IMPLICIT NONE
!
! Altered: 10-Feb-2026 : Changed to V2
!                        PNT_SRCE_MOD, R_PNT_SRCE and NC_PNT added to call.
!                        Added point source flux for ZERO_FLUX inner boudary option.
!                        RSQH_IB_PNT_SRCE is evaluated and passed to moment routines
!                            via MOD_RAY_MOM_STORE.
...
!
!
!	CALL CHECK_VEC_NAN(CHI,ND,'CHI(STOP)',NAN_PRES)
!	CALL CHECK_VEC_NAN(ETA,ND,'ETA(STOP)',NAN_PRES)
!	CALL CHECK_VEC_NAN(ESEC,ND,'ESEC(STOP)',NAN_PRES)
!
	IF(INITIALIZE)THEN
	  NU_ON_dNU=0.0_LDP
	ELSE
...
!
	...
	END IF
!	CALL CHECK_VEC_NAN(CHI_RAY,ND,'CHI_RAY(STOP)',NAN_PRES)
!	CALL CHECK_VEC_NAN(ETA_RAY,ND,'ETA_RAY(STOP)',NAN_PRES)
!
	PAR_MOM=0.0_LDP	
!
...
!
	...
	1               IP,FREQ,NU_ON_dNU,INNER_BND_METH,b_planck,dBdTAU,NRAY,NP,NC)
	    ELSE
              CALL SOLVE_CMF_FORMAL_MPI_V2(CHI_RAY,ETA_RAY,IP,FREQ,NU_ON_dNU,INNER_BND_METH,b_planck,dBdTAU,
	1               RSQH_IB_PNT_SRCE,R_PNT_SRCE,NRAY,NP,NC,NC_PNT)
	    END IF
	  END DO
!
	ELSE
	...
	CALL TUNE(1,'JVAL')
	  ...
	  DO ID=1,MIN(ND,NP-IP+1)
	    ...
	    IF(PAR_Jnu(ID) .NE. PAR_Jnu(ID))THEN
	      WRITE(6,*)'Error in CMF_FORMAL_REL_MPI_V2'
	      WRITE(6,'(4I5,5ES14.4)')MYPE,IPROC,IP, ID, PAR_Jnu(ID),
	1             T1,T2,ray(ip)%Jqw_p(ID),ray(ip)%Jqw_m(ID)
	      CALL SLEEP(2)
	      STOP
	    END IF
	  END DO
...
!
	I=4*ND+16
	CALL MPI_ALLREDUCE(MPI_IN_PLACE,PAR_MOM,I,MY_MPI_DP,MPI_SUM,MPI_COMM_WORLD,IERR)
        JNU_STORE(:)=PAR_MOM(1:ND)
        HNU_STORE(:)=PAR_MOM(ND+1:2*ND)
        KNU_STORE(:)=PAR_MOM(2*ND+1:3*ND)
        NNU_STORE(:)=PAR_MOM(3*ND+1:4*ND)
	CALL MPI_ALLREDUCE(MPI_IN_PLACE,IPLUS,NP,MY_MPI_DP,MPI_SUM,MPI_COMM_WORLD,IERR)
!
        K=4*ND
...
!
	T1=1.0E-200_LDP
!
!	CALL WRITV(JNU,ND,'JNU',720+MYPE); FLUSH(720+MYPE)
	FIRST_TIME=.FALSE.
	RETURN
	END
