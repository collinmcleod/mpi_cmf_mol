...
!
	SUBROUTINE UP_TX_TVX_NOEPS_MPI_V1(TA,TB,TC,PSIPREV_MOD,
	1                       VB,VC,HU,HL,HS,
...
! SIMPTH soluton is now implementd into this routine.
!
...
!
	    I=ND*(VDEND-VDST+1)
	    WRITE(TMP_STR,'(I5)')K; TMP_STR=TRIM(TMP_STR)//'_TX(STOP)'
	    CALL CHECK_VEC_NAN(TX(:,:,K),I,TRIM(TMP_STR),NAN_PRES)
	    I=(ND-1)*(VDEND-VDST+1)
	    WRITE(TMP_STR,'(I5)')K; TMP_STR=TRIM(TMP_STR)//'_TVX(STOP)'
	    CALL CHECK_VEC_NAN(TVX(:,:,K),I,TRIM(TMP_STR),NAN_PRES)
!
