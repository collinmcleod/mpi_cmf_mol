...
!
	IF(USE_HEN_GREEN)THEN
	  ETA_SCAT=0.0_LDP
!$OMP PARALLEL DO SCHEDULE(DYNAMIC) REDUCTION(+:ETA_SCAT) PRIVATE(T1,T2,ID,IP)
	  DO IP=1,NP_LIMIT
	    DO ID=1,MIN(ND,NP-IP+1)
	      T1=RAY(IP)%ETA_P(RAY(IP)%LNK(ID))-ETA_RAY(RAY(IP)%LNK(ID))
	      T2=RAY(IP)%ETA_M(RAY(IP)%LNK(ID))-ETA_RAY(RAY(IP)%LNK(ID))
              ETA_SCAT(ID)=ETA_SCAT(ID)+T1*Jqw_p(ID,ip)+T2*Jqw_m(ID,ip)
	    END DO
	  END DO
	  T1=2.99792458E+03/FREQ
	  IF(T1 .GT. 4000.0 .AND. T1 .LT. 4100)THEN
	    IF(T1 .LT. 4000.2)THEN
	      WRITE(221,*)'Next'
	      ID=1
	      DO IP=1,MAX(NP_LIMIT,NC+ND+1-ID)
	        K=RAY(IP)%LNK(ID)
	        WRITE(221,*)IP,RAY(IP)%MU_M(K),RAY(IP)%ETA_M(K),RAY(IP)%MU_P(K),RAY(IP)%ETA_P(K)
	      END DO
	      WRITE(222,*)'Next'
	      ID=6
	      DO IP=1,MAX(NP_LIMIT,NC+ND+1-ID)
	        K=RAY(IP)%LNK(ID)
	        WRITE(222,*)IP,RAY(IP)%MU_M(K),RAY(IP)%ETA_M(K),RAY(IP)%MU_P(K),RAY(IP)%ETA_P(K)
	      END DO
	    END IF
	    WRITE(179,'(ES14.6,12ES12.4)')T1,(ETA_SCAT(ID),JNU_STORE(ID)*DUST_SCAT_OPAC(ID),ID=1,30,5)
	  END IF
	END IF
!
	ID=ND
