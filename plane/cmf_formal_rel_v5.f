...
!
	  ...
	  REAL(KIND=LDP), ALLOCATABLE :: ETA_P_RAY(:)
	  REAL(KIND=LDP), ALLOCATABLE :: ETA_M_RAY(:)
!
	  INTEGER ND_EXT
	  INTEGER ND_ADD
	  INTEGER NP_MAX
...
!
	END MODULE EXT_REL_GRID_V5
!
	SUBROUTINE CMF_FORMAL_REL_V5
...
!
	...
	1                 SQRT( (R(1)-P(IP))*(R(1)+P(IP)) )/R(1)
	  ...
	  IF( ALLOCATED(ETA_P_RAY) ) DEALLOCATE (ETA_P_RAY)
	  IF( ALLOCATED(ETA_M_RAY) ) DEALLOCATE (ETA_M_RAY)
	  ALLOCATE (CHI_RAY(J),ETA_RAY(J))
	  ALLOCATE (ETA_P_RAY(J),ETA_M_RAY(J))
!
...
!
	IF(RAY_POINTS_INSERTED)THEN
!$OMP PARALLEL DO SCHEDULE(DYNAMIC) PRIVATE(NRAY,I,K,T1,T2,CHI_RAY,ETA_RAY,ETA_M_RAY,ETA_P_RAY)
	  DO IP=1,NP_LIMIT
!
	    NRAY=RAY(IP)%NZ
	    IF(RAY_POINTS_INSERTED)THEN
...
!
	    IF(USE_HEN_GREEN)THEN
	      CALL ADD_ETA_DUST(DUST_SCAT_OPAC,ETA_RAY,ETA_M_RAY,ETA_P_RAY,IP,NRAY,R,ND,NP)
              CALL SOLVE_CMF_FORMAL_V3(CHI_RAY,ETA_M_RAY,ETA_P_RAY,
...
!$OMP PARALLEL DO SCHEDULE(DYNAMIC) PRIVATE(NRAY,ETA_M_RAY,ETA_P_RAY)
	  DO IP=1,NP_LIMIT
	    NRAY=RAY(IP)%NZ
	    IF(USE_HEN_GREEN)THEN
	      CALL ADD_ETA_DUST(DUST_SCAT_OPAC,ETA_RAY,ETA_M_RAY,ETA_P_RAY,IP,NRAY,R,ND,NP)
