...
!
	...
	REAL(KIND=LDP), POINTER :: RSQH_VEC(:)
	REAL(KIND=LDP), POINTER :: TRI_VECS_SAV(:,:)
!
...
!
	SUBROUTINE ALLOC_MOD_JREL_MPI_V1()
	USE SET_KIND_MODULE
	USE MOD_JREL_MPI_V1
	USE MPI
...
!
	  ...
	  IF(IOS .NE. 0)THEN
	    WRITE(LUER,*)'Unable to allocate memory(1) in ALLOC_MOD_JREL_MPI_V1'
	    WRITE(LUER,*)'STAT=',IOS
	    STOP
	  END IF
!
	  ...
	  IF(IOS .NE. 0)THEN
	    WRITE(LUER,*)'Unable to allocate memory(2) in ALLOC_MOD_JREL_MPI_V1'
	    WRITE(LUER,*)'STAT=',IOS
	    STOP
	  END IF
!
...
!
	IF(ND_SAV .EQ. ND)THEN
	  RETURN
	ELSE
	  WRITE(LUER,*)'Error in ALLOC_MOD_JREL_MPI_V1'
...
!
	SUBROUTINE MOM_JREL_MPI_V1(ETA_SM,CHI_SM,ESEC_SM,V_SM,SIGMA_SM,R_SM,
...
!
	  ...
	  ND=K
	  IF(ND .NE. ND_SM)THEN
	    WRITE(LUER,*)'Warning in MOM_JREL_MPI_V1: we have adjusted ND for more accuracy'
...
!
	  IF(N_TYPE .NE. 'G_ONLY' .AND. N_TYPE .NE. 'N_ON_J' .AND. ND .NE. ND_SM)THEN
	    LUER=ERROR_LU()
	    WRITE(LUER,*)'Error in MOM_JREL_MPI_V1'
...
!
          IF(LU_MOM .EQ. 0)CALL GET_LU(LU_MOM,'MOM_JREL_MPI_V1')
...
!
	...
	END IF
!
	SOURCE(DST:DEND)=ETA(DST:DEND)/CHI_J(DST:DEND)
	IF(COHERENT)THEN
	  COH_VEC(DST:DEND)=ESEC(DST:DEND)/CHI_J(DST:DEND)/GAM_REL(DST:DEND)
	ELSE
	  COH_VEC(DST:DEND)=0.0_LDP
	END IF
!
...
!
	MDST=MAX(1,DST-1); MDEND=MIN(DEND,ND-1)
	IF(.NOT. INIT)THEN
!
...
!
	DO I=MAX(2,DST),MIN(DEND,ND-1)
	  TA(I)=-HL(I-1)-EPS(I-1)
	  TC(I)=-HU(I)+EPS(I)
	  TB(I)=DTAUONQ(I)*(1.0_LDP-COH_VEC(I)) + PSI(I) + HL(I) +
...
!
	...
	ELSE
          WRITE(6,*)'Error - XM_CHK_OPTION not recognized in MOM_JREL_MPI_V1'
	...
	IF(DST .EQ. 1)THEN
	  IF(OUTER_BND_METH .EQ. 'HONJ')THEN
	   TC(1)=-( K_ON_J(2)+VdHdR_TERM(2) )*Q(2)/DTAU_H(1)
	    ...
	    VC(1)=0.0_LDP
	  ELSE
	    WRITE(LUER,*)'Error in MOM_JREL_MPI_V1: Only outer boundary method implemented is HONJ'
...
!
	TRI_VECS_SAV(DST:DEND,1)=TA(DST:DEND)
	TRI_VECS_SAV(DST:DEND,2)=TB(DST:DEND)
	TRI_VECS_SAV(DST:DEND,3)=TC(DST:DEND)
...
!
	  ...
	  IF(INCL_ADVEC_TERMS)THEN
	    ...
	    DO I=2,ND-1
	     TB(I)=TB(I)+DTAUONQ(I)*VdJdR_TERM(I)
	    END DO
	  END IF
!
...
!
	  ...
	  DO I=1,ND
	    ...
	    ELSE IF(XM(I) .LT. 0)THEN
	      ...
	      ELSE
                WRITE(6,*)'Error - J_CHK_OPTION not recognized in MOM_JREL_MPI_V1'
...
!
	    ...
	    IF(ICOUNT .EQ.  100)THEN
	      IF(MYPE .EQ. 0)THEN
	        WRITE(LUER,*)'Error in MOM_J_REL_MPI_V1: excessive iteration count.'
