...
!
	...
	1                  THK,INIT,NEW_FREQ,NC,NP,ND,MYPE)
	USE SET_KIND_MODULE
	USE MOD_RAY_MOM_STORE
	USE FG_J_CMF_MOD_MPI_V1
	IMPLICIT NONE
!
	INTEGER NC,NP,ND,MYPE
...
!
	INTEGER GET_LS
	GET_LS(MYPE,NCPU,I)=MOD(I,2)*(MYPE+(I-1)*NCPU+1)+MOD(I+1,2)*(I*NCPU-MYPE)
!
!
	CALL TUNE(1,'FG_FULL')
!
	CALL MPI_COMM_SIZE(MPI_COMM_WORLD,NCPU,IERR)
	NUM_RAYS_PER_CORE=NP/(NCPU-1)+1
!
...
!
	IF(NEW_R_GRID .OR. (INIT .AND. FIRST_TIME))THEN
	  ...
	  CALL SET_ANG_QW_MPI(R,P,NC,ND,NP,MYPE,TRAPFORJ)
	END IF
!
	DO IPROC=1,NUM_RAYS_PER_CORE
	  LS=GET_LS(MYPE,NCPU,IPROC)
	  IF(FIRST_TIME .AND. MYPE .EQ. 0)WRITE(6,*)LS,MYPE,NCPU,IPROC; FLUSH(UNIT=6)
	  IF(LS .GT. NP)EXIT
!
...
!
	...
	END IF
!
	CALL MPI_ALLREDUCE(MPI_IN_PLACE,IPLUS_P,NP,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,ierr)
!
	K=4*ND
...
!	  NNU_AT_IB=DBB/R(ND)/CHI_RAY(NI)/5.0_LDP
	...
	ELSE IF(INNER_BND_METH .EQ. 'PNT_SRCE')THEN
	  ...
	  DO IPROC=1,(NC_PNT_SRCE-1)/NCPU+1
	    LS=GET_LS(MYPE,NCPU,IPROC)
	    IF(LS .GT. NC_PNT_SRCE)EXIT
	    T1=T1+0.5_LDP*IC*RAY(LS)%HQW(ND)
	    T2=T2+0.5_LDP*IC*RAY(LS)%NQW(ND)
	  END DO
	  CALL MPI_ALLREDUCE(T1,HNU_AT_IB,IONE,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,ierr)
	  CALL MPI_ALLREDUCE(T2,NNU_AT_IB,IONE,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,ierr)
	ELSE IF(INNER_BND_METH .EQ. 'ZERO_FLUX')THEN
	  HNU_AT_IB=0.0_LDP
	  NNU_AT_IB=0.0_LDP
	ELSE
	  T1=0.0_LDP; T2=0.0_LDP; HNU_AT_IB=0.0_LDP; NNU_AT_IB=0.0_LDP
	  DO IPROC=1,NC/NCPU+1
	    LS=GET_LS(MYPE,NCPU,IPROC)
	    IF(LS .GT. NC+1)EXIT
...
!
	...
	END DO
	CALL MPI_ALLREDUCE(T1,IN_HBC,IONE,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,ierr)
!
