...
!
        REAL(KIND=LDP), SAVE :: T0,T1,T2,dT,OVERHEAD
...
!
        REAL(KIND=LDP), SAVE :: ST_MEM(MAX_IDS)
        REAL(KIND=LDP), SAVE :: dM_MAX(MAX_IDS)
	REAL(KIND=LDP) MAX_MEM,MAX_MEM_BYTES
	CHARACTER(LEN=3) MEM_UNIT,TMP_UNIT
!
	INTEGER LUOUT
	INTEGER CURRENT_ID
	INTEGER ACTIVE_ID
...
!
	...
	IMPLICIT NONE
!
! Altered 08-Mar-2025 : Added option to print out memory diagnostics.
...
!                         Replaces the less general routine (now called tune_mp0.f).
...
!
	...
	END IF
!
	IF(DO_MEM_DIAGNOSTICS)THEN
	  CALL GET_MAX_MEMORY(MAX_MEM_BYTES,MAX_MEM,MEM_UNIT,' ',IZERO)
	END IF
!
	IF(FIRSTTIME)THEN
          FIRSTTIME=.FALSE.
...
!
	  IF(CURRENT_ID .NE. 0)THEN
	    ...
	    dM_MAX(I)=MAX(dM_MAX(I),MAX_MEM_BYTES-ST_MEM(I))
...
C
	ELSE IF (LRUN .EQ. 3) THEN
	  ...
	  IF(DO_MEM_DIAGNOSTICS)THEN
	    DO I=1,MAX_IDS
	      IF (IDLIST(I).EQ.' ') EXIT
	      CALL CNVT_MEM_UNIT(ST_MEM(I),T1,MEM_UNIT)
	      CALL CNVT_MEM_UNIT(dM_MAX(I),T2,TMP_UNIT)
	      WRITE(LUOUT,'(1X,a24,f15.6,2X,f15.6,2X,2(F11.3,A))')
	1     IDLIST(I),WALLTOT(I),CPUTOT(I),T1,MEM_UNIT,T2,TMP_UNIT
	    END DO
	  ELSE
	    DO I=1,MAX_IDS
	      IF (IDLIST(I).EQ.' ') EXIT
