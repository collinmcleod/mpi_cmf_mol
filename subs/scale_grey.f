...
!
	...
	IMPLICIT NONE
!
! Altered 23-Mar-2026: Fixed bug that caused hang when when GREY_SCL_FAC_IN is not available.
...
1000	CONTINUE
!
! Indicate to all processes whether GREY computation by process 0 was successful.
!
	CALL MPI_BCAST(IOS,IONE,MPI_INTEGER,IZERO,MPI_COMM_WORLD,IERR)
	IF(IOS .EQ. 0)THEN
	   CALL MPI_BCAST(TGREY,ND,MY_MPI_DP,IZERO,MPI_COMM_WORLD,IERR)
	END IF
!
	RETURN
	END
