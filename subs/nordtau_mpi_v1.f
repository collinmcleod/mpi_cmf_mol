...
C          rule is recovered.
C
	SUBROUTINE  NORDTAU_MPI_V1(DTAU,CHI,Z,R,dCHIdR,DST,DEND,NI)
	USE SET_KIND_MODULE
	IMPLICIT NONE
C
...
C the case.
C
	SUBROUTINE DERIVCHI_MPI_V1(dCHIdr,CHI,R,DST,DEND,ND,METHOD)
	USE SET_KIND_MODULE
	IMPLICIT NONE
C
...
C
	...
	1               LOG(R(I-1)/R(I+1)) *CHI(I)/R(I)
	  ...
	  END IF
	  IF((DEND-2)*(2-DST) .GE. 0)THEN
	    IF( (R(1)-R(3))/(R(1)-R(2)) .GT. LIM )THEN
	      T1=LOG(CHI(1)/CHI(2)) / LOG(R(1)/R(2))
	      dCHIdR(2)=T1*CHI(2)/R(2)
	    END IF
	  END IF
	  IF(DEND .EQ. ND)THEN
	  ...
	  END IF
	  IF( (DEND-ND+1)*(ND-1-DST) .GE. 0)THEN
	     IF( (R(ND-2)-R(ND))/(R(ND-1)-R(ND)) .GT. LIM )THEN
	       T1=LOG(CHI(ND-1)/CHI(ND)) / LOG(R(ND-1)/R(ND))
	       dCHIdR(ND-1)=T1*CHI(ND-1)/R(ND-1)
	     END IF
	  END IF
!
!
	ELSE IF(METHOD .EQ. 'LOGLIN')THEN
	  DO I=MAX(2,DST),MIN(ND-1,DEND)
	    dCHIdR(I)=(LOG(CHI(I-1))-LOG(CHI(I+1)))/(R(I-1)-R(I+1))*CHI(I)
	  END DO
	  IF(DST .EQ. 1)dCHIdR(1)=CHI(1)*(LOG(CHI(1))-LOG(CHI(2)))/(R(1)-R(2))
	  IF( (2-DST)*(DEND-2) .GE. 0)THEN
	    IF( (R(1)-R(3))/(R(1)-R(2)) .GT. LIM )THEN
	       dCHIdR(2)=CHI(2)*(LOG(CHI(1))-LOG(CHI(2)))/(R(1)-R(2))
	    END IF
	  END IF
	  IF(DEND .EQ. 1)dCHIdR(ND)= CHI(ND)*(LOG(CHI(ND-1))-LOG(CHI(ND)))/(R(ND-1)-R(ND))
	  IF((ND-1-DST)*(DEND-ND+1) .GE. 0)THEN
	    IF( (R(ND-2)-R(ND))/(R(ND-1)-R(ND)) .GT. LIM )THEN
	      dCHIdR(ND-1)=CHI(ND-1)*(LOG(CHI(ND-1))-LOG(CHI(ND)))/(R(ND-1)-R(ND))
	    END IF
	  END IF
!
!
	ELSE IF(METHOD .EQ. 'LINEAR')THEN
	  DO I=MAX(DST,2),MIN(ND-1,DEND)
	  ...
	  END IF
	  IF( (2-DST)*(DEND-2) .GE. 0)THEN
	    IF( (R(1)-R(3))/(R(1)-R(2)) .GT. LIM ) dCHIdR(1)=(CHI(1)-CHI(2))/(R(1)-R(2))
	  END IF
	  IF(DEND .EQ. ND)dCHIdR(ND)=(CHI(ND-1)-CHI(ND))/(R(ND-1)-R(ND))
	  IF( (ND-1-DST)*(DEND-ND+1) .GE. 0)THEN
	     IF( (R(ND-2)-R(ND))/(R(ND-1)-R(ND)) .GT. LIM )dCHIdR(ND-1)=(CHI(ND-1)-CHI(ND))/(R(ND-1)-R(ND))
	  END IF
!
!
...
C
	  ...
	  END DO
	  IF(DEND .EQ. ND .AND. DST .EQ. DEND)H(ND-2)=R(ND-1)-R(ND-2)
C
	  IF(DST .EQ. 1)THEN
...
!
	  ...
	  END DO
	  IF(DEND .EQ. ND .AND. DST .EQ. DEND)H(ND-2)=LOG(R(ND-1)/R(ND-2))
!
	  IF(DST .EQ. 1)THEN
            dCHIdR(1)=SLOPE(1) +(SLOPE(1)-SLOPE(2))*H(1)/(H(1)+H(2))
	  END IF
...
C 
C
	SUBROUTINE d_DERIVCHI_dCHI_MPI_V1(dCHIdr,CHI,R,DST,DEND,ND,METHOD)
	USE SET_KIND_MODULE
