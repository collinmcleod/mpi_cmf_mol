...
!
	SUBROUTINE CNVT_FR_DC_V2(C2,LOG_C2LTE,DC2,NC2,DCI,SUM,DST,DEND,ND,FIRST,CIII_PRES)
	USE SET_KIND_MODULE
	IMPLICIT NONE
!
...
!
	...
	INTEGER NC2,DST,DEND,ND
	REAL(KIND=LDP) C2(NC2,DST:DEND)
	REAL(KIND=LDP) LOG_C2LTE(NC2,DST:DEND)
	REAL(KIND=LDP) DC2(DST:DEND)
	REAL(KIND=LDP) DCI(ND)
	REAL(KIND=LDP) SUM(ND)
!
	INTEGER I,J
!
	IF(FIRST)THEN
