...
!
	...
	1                     HN,HNST,NLEV,ZHYD,
	1                     DI,LOG_DIST,N_DI,
	1                     ION_LEVELS,NPHOT, ED,T,
	1                     JREC,JPHOT,JREC_CR,JPHOT_CR,BPHOT_CR,
	1                     NU,NU_CONT,INIT_ARRAYS,DST,DEND,ND)
	USE SET_KIND_MODULE
	IMPLICIT NONE
!
...
!
	...
	INTEGER DST,DEND
!
	REAL(KIND=LDP) PR(NLEV,DST:DEND),RR(NLEV,DST:DEND)
	REAL(KIND=LDP) BFCR(NLEV,DST:DEND),FF(DST:DEND)
	REAL(KIND=LDP) HN(NLEV,DST:DEND),HNST(NLEV,DST:DEND)
	REAL(KIND=LDP) WSE(NLEV,DST:DEND,NPHOT),WCR(NLEV,DST:DEND,NPHOT)
	REAL(KIND=LDP) DI(N_DI,DST:DEND),LOG_DIST(N_DI,DST:DEND)
!
...
!
	...
	INTEGER NPHOT
	INTEGER ION_LEVELS(NPHOT)		!Final (destination) level in ion.
!
...
!
	...
	INTEGER I,K
	INTEGER ION_LEV
	INTEGER PHOT_ID,IP   				!Photoionization ID
...
!
! If ML=1 then initialize all arrays. This routine
...
!
	DO PHOT_ID=1,NPHOT
	  ...
	  DO K=DST,DEND
	    ...
	    END IF
	    DO I=1,NLEV
	      IF(WSE(I,K,IP) .NE. 0)THEN
	        PR(I,K)=PR(I,K)+WSE(I,K,IP)*HN(I,K)*JPHOT(K)
	        RR(I,K)=RR(I,K)+WSE(I,K,IP)*HNST(I,K)*JB_RAT
	        BFCR(I,K)=BFCR(I,K)+
	1            ( HNST(I,K)*(WCR(I,K,IP)*JB_RAT+WSE(I,K,IP)*JC_RAT)
	1               -HN(I,K)*(WCR(I,K,IP)*JPHOT(K)+WSE(I,K,IP)*JPHOT_CR(K)) )*H
	      END IF
	    END DO
	  END DO
!
