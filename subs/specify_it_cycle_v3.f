...
!
	...
	IMPLICIT NONE
!
! Aletere 09-Aug-2025: Adapted for MPI
...
!
	...
	LOGICAL FILE_EXISTS
!
	INQUIRE(FILE='IT_SPECIFIER',EXIST=FILE_EXISTS)
	IF(.NOT. FILE_EXISTS)RETURN
!
	LU_IN=7
	OPEN(UNIT=LU_IN,FILE='IT_SPECIFIER',STATUS='OLD',IOSTAT=IOS,ACTION='READ')
	IF(IOS .NE. 0)RETURN
!
	K=0
...
!
	...
	DO WHILE(CNT .LT. CYCLE_LENGTH)
	  ...
	  ELSE IF(STORE(1)(1:8) .EQ. 'REVISE_R')THEN
	    ...
	    STORE(1:CYCLE_LENGTH-1)=STORE(2:CYCLE_LENGTH)
	    STORE(CYCLE_LENGTH)=TEMP_STR
	    CALL GET_LU(K,'SPECIFY_IT_CYCLE_V3')
	    IF(MYPE .EQ. 0)THEN
	      I=MAIN_COUNTER+1
...
!
	...
	ELSE
	  ...
	  IF(MYPE .EQ. 0)THEN
	    ...
	    OPEN(UNIT=LU_IN,FILE='IT_SPECIFIER',STATUS='OLD')
	    DO I=2,CYCLE_LENGTH
...
!
	ITS_DONE=ITS_DONE+1
	CLOSE(LU_IN)
	IF(ITS_TO_DO .EQ. ITS_DONE .AND. MYPE .EQ. 0)THEN
	  OPEN(UNIT=LU_IN,FILE='IT_SPECIFIER',STATUS='OLD')
	    DO I=2,CYCLE_LENGTH
	      WRITE(LU_IN,'(A)')TRIM(STORE(I))
	    END DO
