...
!******************************************************************************
!
	SUBROUTINE SUB_PHOT_GEN_MPI_V2(ID,PHOT,FREQ,GS_EDGE,NLEVS,PHOT_ID,SET_TO_EDGE)
	USE SET_KIND_MODULE
!
...
!
	...
	DO I=1,NLEVS
	  ...
	  IF(SET_TO_EDGE)FREQ_VEC(I)=EDGE
	END DO
!
! 
!
...
!
	IF(SET_TO_EDGE)THEN
	  PD(ID)%EDGE_CROSS(1:NLEVS,PHOT_ID)=PHOT(1:NLEVS)
	  PD(ID)%EDGE_FREQ(1:NLEVS,PHOT_ID)=FREQ_VEC(1:NLEVS)
 	ELSE
