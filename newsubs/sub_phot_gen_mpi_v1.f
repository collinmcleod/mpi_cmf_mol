...
!******************************************************************************
!
	SUBROUTINE SUB_PHOT_GEN_MPI_V1(ID,PHOT,FREQ,GS_EDGE,NLEVS,PHOT_ID,SET_TO_EDGE)
	USE SET_KIND_MODULE
!
...
!
	REAL(KIND=LDP), SAVE, POINTER :: LOC_PHOT(:)
	INTEGER, SAVE :: PHOT_WIN
	LOGICAL, SAVE :: SHARED_MEM_ALLOCATED=.FALSE.
!
...
!
	CALL TUNE(1,'SUB_PHOT')
	CALL TUNE(1,'PHOT_SIMP')
	IF(PHOT_ID .LT. 0)THEN
	  ...
	  ELSE
	    ...
	    STOP
	  END IF
	  CALL TUNE(2,'PHOT_SIMP')
	  CALL TUNE(2,'SUB_PHOT')
	  RETURN
	END IF
!
...
!
	...
	IF(SET_TO_EDGE)THEN
	  DO I=1,NLEVS
	    ...
	    IF(FREQ .LT. EDGE)FREQ_VEC(I)=EDGE
	  END DO
	END IF
!
...
!
	ALL_DONE=.TRUE.
	DO I=1,NLEVS
	  IF(FREQ_VEC(I) .NE. PD(ID)%LST_FREQ(I,PHOT_ID))THEN
	    ALL_DONE=.FALSE.
	    EXIT
	  END IF
	END DO
...
!
	IF(.NOT. SHARED_MEM_ALLOCATED)THEN
	  ...
	  CALL SUB_ALLOCATE_DP_MPI_MEM(LOC_PHOT,2000,MYPE,0,MPI_COMM_WORLD,PHOT_WIN)
	  SHARED_MEM_ALLOCATED=.TRUE.
...
!
	NLEVS_PER_THRD=(NLEVS-1)/NTHREAD+1
	DO I=1+MYPE,NLEVS,NLEVS_PER_THRD
          IF(I .GT. NLEVS)EXIT
	  LOC_PHOT(I)=0.0_LDP
	END DO
	CALL MPI_WIN_FENCE(0,PHOT_WIN,IERR)
...
C
!	    DO I=NLEVS-MYPE,1,-NLEVS_PER_THRD
!	      IF(I .LT. 1)EXIT
!
	    DO I=1+MYPE,NLEVS,NLEVS_PER_THRD
	      IF(I .GT. NLEVS)EXIT
...
!
	...
	1                   (RJ-N_PER_L)+BF_L_CROSS(J)
		    ...
		    J=RJ+1 		!Restore as corrupted	
	          ...
	          LOC_PHOT(I)=LOC_PHOT(I) + SUM/( (LEND-LST+1)*(LEND+LST+1) )
!
! Hydrogenic transitions.
!
...
!
	          ...
	          IF(U .GE. PD(ID)%NU_NORM(N))THEN
	            LOC_PHOT(I)=LOC_PHOT(I)+CONV_FAC*PD(ID)%CROSS_A(N)*(PD(ID)%NU_NORM(N)/U)**3
...
!
	            ...
	            LOC_PHOT(I)=LOC_PHOT(I)+ CONV_FAC*(
...
!
	          LOC_PHOT(I)=LOC_PHOT(I) + PD(ID)%ALPHA_BF*T1*PD(ID)%CROSS_A(LMIN)/
...
!
	...
	1                   RU*(PD(ID)%CROSS_A(LMIN+4)+RU*PD(ID)%CROSS_A(LMIN+5)))) )  )
	          IF(T1 .GT. 0.0_LDP)LOC_PHOT(I)=LOC_PHOT(I)+T1
!
!
...
!
	...
	1                  PD(ID)%CROSS_A(LMIN+1))*X+PD(ID)%CROSS_A(LMIN)
	            ...
	            END IF
	            LOC_PHOT(I)=LOC_PHOT(I)+10.0_LDP**(T1+LG10_CONV_FAC)
	          END IF
!
! Modified seaton fit.
!
	        ...
	        ELSE IF(PD(ID)%CROSS_TYPE(TERM,GRID_ID) .EQ. 9)THEN
	          ...
	          DO J=1,(PD(ID)%END_LOC(TERM,GRID_ID)-PD(ID)%ST_LOC(TERM,GRID_ID)+1)/8
	            ...
	            IF(FREQ_VEC(I) .GE. EDGE)THEN
                      ...
                      LOC_PHOT(I)=LOC_PHOT(I)+1.0E-08_LDP*T1*PD(ID)%CROSS_A(LMIN+4)/T2/T3
                    END IF
	          END DO
!
!
!
	        ELSE IF(PD(ID)%CROSS_TYPE(TERM,GRID_ID) .EQ. 30)THEN
	          ...
	          IF(T1 .GT. 0)LOC_PHOT(I)=LOC_PHOT(I)+T1
...
! included.
!
	ADDITIONAL_OPAC=0.0_LDP
	IF(PHOT_ID .EQ. 1 .AND. FREQ .GT. 0)THEN
	  IF(PD(ID)%DO_KSHELL_W_GS)THEN
...
!
	  ...
	  IF(INE .EQ. 11)THEN
	    ...
	    ADDITIONAL_OPAC=ADDITIONAL_OPAC+T2
	  END IF
!
...
!
	MIN_LOC=0; MAX_LOC=0
	IF(ADDITIONAL_OPAC .GT. 0.0_LDP)THEN
	  DO I=1+MYPE,NLEVS,NLEVS_PER_THRD
	    IF(I .GT. NLEVS)EXIT
	    LOC_PHOT(I)=(LOC_PHOT(I)+ADDITIONAL_OPAC)*PD(ID)%SCALE_FAC(PHOT_ID)
	    IF(LOC_PHOT(I) .GT. 0.0_LDP .AND. MIN_LOC .EQ. 0)MIN_LOC=I
	    IF(LOC_PHOT(I) .GT. 0.0_LDP)MAX_LOC=I
	  END DO
	ELSE
	  DO I=1+MYPE,NLEVS,NLEVS_PER_THRD
	    IF(I .GT. NLEVS)EXIT
	    LOC_PHOT(I)=LOC_PHOT(I)*PD(ID)%SCALE_FAC(PHOT_ID)
	    IF(LOC_PHOT(I) .GT. 0.0_LDP .AND. MIN_LOC .EQ. 0)MIN_LOC=I
	    IF(LOC_PHOT(I) .GT. 0.0_LDP)MAX_LOC=I
	  END DO
	END IF
